...
function ManagingCommittee({rows,add,updateRecord,archive,token}){
 ...
 const current=[
  ...
  {memberId:"SSF-MBR-00004",memberType:"Founder Member",designation:"Joint Secretary",functionalResponsibility:"IT, MIS & Digital Records",fullName:"Kiran Pandey",occupation:"Homemaker & Social Worker",gender:"Female",mobile:"9993495877",email:"kiranpandey1729@gmail.com",address:"26/282, Ambedkar Nagar, Pokhri tola",city:"Rewa",state:"Madhya Pradesh",pinCode:"486005",joiningDate:"2013-12-30",effectiveFrom:"2013-12-30",appointmentDate:"2013-12-30"},
  {memberId:"SSF-MBR-00015",memberType:"General Member",designation:"Executive Committee Member",functionalResponsibility:"Documentation Head (Admin In-charge)",fullName:"Sandeep Tripathi",occupation:"Teacher & Social Worker",gender:"Male",mobile:"7697851754",email:"tri.sandeep22@gmail.com",address:"Village Balha, Post Nayagaon",city:"Satna",state:"Madhya Pradesh",pinCode:"485221",joiningDate:"2025-05-10",effectiveFrom:"2025-05-10",appointmentDate:"2025-05-10"},
  {memberId:"SSF-MBR-00016",memberType:"General Member",designation:"Member",functionalResponsibility:"Field Coordinator",fullName:"Prameesh Singh",occupation:"Fitness Trainer",gender:"Male",mobile:"9144796001",email:"prameeshs321@gmail.com",address:"Village Khaira, Khaira",city:"Rewa",state:"Madhya Pradesh",pinCode:"486441",joiningDate:"2025-05-10",effectiveFrom:"2025-05-10",appointmentDate:"2025-05-10"},
  {memberId:"SSF-MBR-00017",memberType:"General Member",designation:"Member",functionalResponsibility:"Volunteer Coordinator",fullName:"Rishi Kumar Pandey",occupation:"Private Employee",gender:"Male",mobile:"7987707912",email:"rishisatna01@gmail.com",address:"Village-Post Kyoti",city:"Rewa",state:"Madhya Pradesh",pinCode:"486117",joiningDate:"2025-05-10",effectiveFrom:"2025-05-10",appointmentDate:"2025-05-10"},
  {memberId:"SSF-MBR-00018",memberType:"General Member",designation:"Member",functionalResponsibility:"Media & Communication Coordinator",fullName:"Ritesh Kumar Tiwari",occupation:"Private Employee",gender:"Male",mobile:"8422819534",email:"riteshtiwari9082@gmail.com",address:"Village Jagannathpur, Sant Ravidas Nagar",city:"Bhadohi",state:"Uttar Pradesh",pinCode:"221303",joiningDate:"2025-05-10",effectiveFrom:"2025-05-10",appointmentDate:"2025-05-10"}
 ];
 const [f,setF]=useState(blank),[editingId,setEditingId]=useState(null),[section,setSection]=useState("dashboard"),[saving,setSaving]=useState(false),[notice,setNotice]=useState("");
 const committeeRows=(rows||[]).filter(r=>r.module==="managingCommittee"&&r.status!=="deleted");
 const active=committeeRows.filter(r=>String(r.status).toLowerCase()==="active");
 const findMember=id=>committeeRows.find(r=>String((r.data||{}).memberId)===id);
 const refreshCurrent=async()=>{
  setNotice("Refreshing current Managing Committee…");
  for(const m of current){
   const r=findMember(m.memberId);
   const data={...m,status:"Active",responsibilities:m.functionalResponsibility,action:"Current Committee Register"};
   if(r) await updateRecord(r.id,"managingCommittee",data);
   else await add("managingCommittee",{recordDate:m.effectiveFrom,recordType:"Committee Member",status:"active",data});
  }
  const keep=new Set(current.map(m=>m.memberId));
  for(const r of committeeRows){
   const id=String((r.data||{}).memberId||"");
   if(id&&!keep.has(id)) await archive(r.id);
  }
  setNotice("Current Managing Committee synchronized: 9 members.");
 };
 useEffect(()=>{if(token&&committeeRows.length===0)refreshCurrent();},[token]);
 const editRecord=r=>{setEditingId(r.id);setF({...blank,...(r.data||{})});setSection("profile");setNotice("");window.scrollTo({top:0,behavior:"smooth"});};
 const reset=()=>{setEditingId(null);setF({...blank});setSection("dashboard");};
 const save=async e=>{
  e.preventDefault(); if(saving)return;
  if(!String(f.fullName||"").trim()){setNotice("Full Name is required.");return;}
  if(!f.designation){setNotice("Organization Role is required.");return;}
  setSaving(true);
  const data={...f,status:f.status||"Active",action:"Committee Register Update"};
  const ok=editingId?await updateRecord(editingId,"managingCommittee",data):await add("managingCommittee",{recordDate:f.effectiveFrom||new Date().toISOString().slice(0,10),recordType:"Committee Member",status:"active",data});
  setSaving(false);
  if(ok){setNotice(editingId?"Committee member updated successfully.":"Committee member added successfully.");setEditingId(null);}
 };
 ...
 const field=(k,label,type="text")=><div><label className="text-xs font-bold text-slate-600">{label}</label><input type={type} value={f[k]||""} onChange={e=>setF(x=>({...x,[k]:e.target.value}))} className={cls+" mt-1"}/></div>;
 const renderForm=<form onSubmit={save} className="space-y-5">
  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{field("memberId","Member ID")}{field("memberType","Member Type")}{field("fullName","Full Name")}{field("occupation","Occupation / Profession")}{field("gender","Gender")}{field("mobile","Mobile No.")}{field("email","Email")}{field("address","Address")}{field("city","City")}{field("state","State")}{field("pinCode","PIN Code")}{field("joiningDate","Membership Joining Date","date")}</div>
  <div className="border-t pt-5"><h4 className="font-black text-[#002344] mb-3">Position & Responsibility</h4><div className="grid sm:grid-cols-2 gap-4"><div><label className="text-xs font-bold text-slate-600">Organization Role</label><select value={f.designation} onChange={e=>setF(x=>({...x,designation:e.target.value}))} className={cls+" mt-1"}>{roles.concat(["Executive Committee Member","Member","Other / Custom"]).map(x=><option key={x}>{x}</option>)}</select></div>{field("functionalResponsibility","Functional Responsibility")}</div><label className="block text-xs font-bold text-slate-600 mt-4">Responsibilities / Duties</label><textarea value={f.responsibilities||""} onChange={e=>setF(x=>({...x,responsibilities:e.target.value}))} className={cls+" mt-1 min-h-[110px]"}/></div>
  <div className="border-t pt-5"><h4 className="font-black text-[#002344] mb-3">Appointment, Tenure & Governance</h4><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">{field("effectiveFrom","Committee From","date")}{field("validTill","Committee Till","date")}{field("appointmentDate","Appointment / Selection Date","date")}{field("meetingDate","Meeting Date","date")}{field("referenceNo","Reference / File No.")}{field("resolutionNo","Resolution No.")}<div><label className="text-xs font-bold text-slate-600">Committee Status</label><select value={f.status||"Active"} onChange={e=>setF(x=>({...x,status:e.target.value}))} className={cls+" mt-1"}>{["Active","Ended","Role Changed","Resigned","Removed"].map(x=><option key={x}>{x}</option>)}</select></div></div><label className="block text-xs font-bold text-slate-600 mt-4">Remarks</label><textarea value={f.remarks||""} onChange={e=>setF(x=>({...x,remarks:e.target.value}))} className={cls+" mt-1 min-h-[80px]"}/></div>
  <div className="flex flex-wrap gap-3"><button type="submit" disabled={saving} className="bg-[#002344] text-white px-6 py-3 rounded-xl font-bold">{saving?"Saving…":editingId?"Update Committee Member":"Save Committee Member"}</button>{editingId&&<button type="button" onClick={reset} className="border px-6 py-3 rounded-xl font-bold">Cancel Edit</button>}</div>
 </form>;
 const dashboard=<div className="space-y-5"><div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[["Current Members",active.length],["Founder Members",active.filter(r=>(r.data||{}).memberType==="Founder Member").length],["Key Roles",active.filter(r=>roles.includes((r.data||{}).designation)).length],["History Records",committeeRows.length]].map(x=><div key={x[0]} className="bg-slate-50 border rounded-2xl p-5"><div className="text-xs font-bold text-slate-500">{x[0]}</div><div className="text-3xl font-black text-[#002344] mt-1">{x[1]}</div></div>)}</div><div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">{roles.map(role=>{const r=active.find(x=>(x.data||{}).designation===role);return <button type="button" key={role} onClick={()=>r&&editRecord(r)} className="text-left border rounded-2xl p-4 hover:shadow-md hover:border-[#1F7A70] bg-white"><div className="text-xs font-black text-[#1F7A70]">{role}</div><div className="font-black text-[#002344] mt-2">{r?.data?.fullName||"Vacant"}</div><div className="text-xs text-slate-500 mt-1">{r?.data?.functionalResponsibility||"—"}</div></button>})}</div><div className="bg-white border rounded-2xl p-5"><div className="flex flex-wrap justify-between items-center gap-3"><div><h3 className="text-xl font-black text-[#002344]">Current Committee Members</h3><p className="text-sm text-slate-500">Click Edit on any member to update the official record.</p></div><button type="button" onClick={()=>setSection("register")} className="bg-[#123B5D] text-white px-4 py-2 rounded-xl font-bold">Open Complete Register</button></div><div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3 mt-4">{active.map(r=>{const d=r.data||{};return <div key={r.id} className="border rounded-xl p-4"><div className="font-black text-[#002344]">{d.fullName||"—"}</div><div className="text-sm font-bold text-[#1F7A70] mt-1">{d.designation||"Member"}</div><div className="text-xs text-slate-500 mt-2">{d.functionalResponsibility||"—"}</div><button type="button" onClick={()=>editRecord(r)} className="mt-3 px-3 py-1.5 rounded-lg bg-[#002344] text-white text-sm font-bold">Edit</button></div>})}</div></div></div>;
 const history=<div className="overflow-auto"><table className="w-full text-sm min-w-[900px]"><thead className="bg-slate-50"><tr>{["Date","Member","Role","Action","Status","Resolution","Meeting","Reference","Action"].map(h=><th key={h} className="p-3 text-left">{h}</th>)}</tr></thead><tbody className="divide-y">{committeeRows.slice().sort((a,b)=>String(b.recordDate).localeCompare(String(a.recordDate))).map(r=>{const d=r.data||{};return <tr key={r.id}><td className="p-3">{formatOfficeDate(r.recordDate||d.effectiveFrom)}</td><td className="p-3 font-bold">{d.fullName||"—"}</td><td className="p-3">{d.designation||"—"}</td><td className="p-3">{d.action||r.recordType||"Committee Register Update"}</td><td className="p-3">{r.status}</td><td className="p-3">{d.resolutionNo||"—"}</td><td className="p-3">{d.meetingDate||"—"}</td><td className="p-3">{d.referenceNo||"—"}</td><td className="p-3 whitespace-nowrap"><button type="button" onClick={()=>editRecord(r)} className="px-3 py-1.5 rounded-lg border border-[#1F7A70] text-[#1F7A70] font-bold mr-2">Edit</button><button type="button" onClick={()=>doArchive(r.id)} className="px-3 py-1.5 rounded-lg border border-red-200 text-red-700 font-bold">Archive</button></td></tr>})}</tbody></table></div>;
 const actions=<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{["Role Change / Transfer","Additional Responsibility","Responsibility Withdrawal","Resignation","Removal / Membership Cancellation","Replacement / Relieving"].map(a=><button key={a} type="button" onClick={()=>{if(!editingId){setNotice("First click Edit on the member concerned, then record the governance action.");return;}add("managingCommittee",{recordDate:f.effectiveFrom||new Date().toISOString().slice(0,10),recordType:a,status:"active",data:{memberId:f.memberId,fullName:f.fullName,designation:f.designation,action:a,effectiveFrom:f.effectiveFrom,resolutionNo:f.resolutionNo,meetingDate:f.meetingDate,referenceNo:f.referenceNo,remarks:f.remarks}}).then(()=>setNotice(a+" recorded."));}} className="border rounded-xl p-4 text-left font-bold hover:border-[#1F7A70] hover:bg-slate-50">{a}</button>)}</div>;
 const register=<div className="overflow-auto"><table className="w-full text-sm min-w-[2300px]"><thead className="bg-slate-50"><tr>{["Member ID","Member Type","Organization Role","Functional Responsibility","Full Name","Occupation","Gender","Mobile","Email","Address","City","State","PIN","Joining Date","Committee From","Committee Till","Status","Appointment Date","Reference","Resolution","Meeting Date","Responsibilities","Remarks","Action"].map(h=><th key={h} className="p-3 text-left whitespace-nowrap">{h}</th>)}</tr></thead><tbody className="divide-y">{committeeRows.slice().sort((a,b)=>String(b.recordDate).localeCompare(String(a.recordDate))).map(r=>{const d=r.data||{};return <tr key={r.id}>{[d.memberId,d.memberType,d.designation,d.functionalResponsibility,d.fullName,d.occupation,d.gender,d.mobile,d.email,d.address,d.city,d.state,d.pinCode,d.joiningDate,d.effectiveFrom,d.validTill,d.status||r.status,d.appointmentDate,d.referenceNo,d.resolutionNo,d.meetingDate,d.responsibilities,d.remarks].map((v,i)=><td key={i} className="p-3 align-top max-w-[260px] whitespace-pre-wrap">{v||"—"}</td>)}<td className="p-3 whitespace-nowrap"><button type="button" onClick={()=>editRecord(r)} className="px-3 py-1.5 rounded-lg bg-[#002344] text-white font-bold mr-2">Edit</button><button type="button" onClick={()=>doArchive(r.id)} className="px-3 py-1.5 rounded-lg border border-red-200 text-red-700 font-bold">Archive</button></td></tr>})}</tbody></table>{!committeeRows.length&&<div className="p-8 text-center text-slate-500">No committee records found.</div>}</div>;
 return <div className="space-y-5"><div className="bg-gradient-to-r from-[#002344] via-[#123B5D] to-[#1F7A70] text-white rounded-2xl p-6"><div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4"><div><div className="text-xs uppercase tracking-[.2em] text-white/60 font-bold">SSF Digital Office · Governance</div><h2 className="text-3xl font-black mt-2">Managing Committee <span className="text-white/70">/ प्रबंधकारिणी समिति</span></h2><p className="text-white/80 mt-2">Current committee, responsibilities, appointments, tenure and governance history.</p></div><div className="flex gap-2 flex-wrap"><button type="button" onClick={refreshCurrent} className="bg-white text-[#002344] px-4 py-2.5 rounded-xl font-bold">Refresh Current Data</button><button type="button" onClick={()=>{setEditingId(null);setF({...blank,...current[0]});setSection("profile");}} className="bg-white/15 border border-white/30 text-white px-4 py-2.5 rounded-xl font-bold">Add / New Record</button></div></div></div>{notice&&<div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl p-3 font-semibold">{notice}</div>}<div className="bg-white border rounded-2xl p-2 overflow-x-auto"><div className="flex gap-2 min-w-max">{tabs.map(([id,label])=><button type="button" key={id} onClick={()=>setSection(id)} className={"px-4 py-2.5 rounded-xl text-sm font-bold "+(section===id?"bg-[#002344] text-white":"bg-slate-100 text-[#123B5D] hover:bg-slate-200")}>{label}</button>)}</div></div><div className="bg-white border rounded-2xl p-5">{section==="dashboard"?dashboard:section==="profile"?renderForm:section==="position"?<div>{renderForm}</div>:section==="appointment"?<div>{renderForm}</div>:section==="tenure"?<div>{renderForm}</div>:section==="history"?history:section==="actions"?actions:register}</div></div>;
}
