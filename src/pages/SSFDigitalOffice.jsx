...
function InstitutionalHistory({rows,add,archive}){
 ...
 return <SimpleOfficeCard title="🏛️ संस्था इतिहास (Institutional History)" subtitle="2013 से आज तक SSF की master institutional timeline. पुराने records overwrite नहीं होंगे."><div className="bg-white border rounded-2xl p-5"><form onSubmit={save} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3"><input type="date" value={f.date} onChange={e=>set("date",e.target.value)} className={cls}/><select value={f.eventType} onChange={e=>set("eventType",e.target.value)} className={cls}>{["Institution Formation","Registration","Committee Formation / Reconstitution","Organisational Change","Important Decision","Important Project / Initiative","Compliance / Registration","MoU / Partnership","Other"].map(x=><option key={x}>{x}</option>)}</select><input value={f.title} onChange={e=>set("title",e.target.value)} placeholder="Event Title" required className={cls}/><input value={f.personCommittee} onChange={e=>set("personCommittee",e.target.value)} placeholder="Person / Committee" className={cls}/><input value={f.previousRole} onChange={e=>set("previousRole",e.target.value)} placeholder="Previous Position (if any)" className={cls}/><input value={f.newRole} onChange={e=>set("newRole",e.target.value)} placeholder="New Position (if any)" className={cls}/><input value={f.referenceNo} onChange={e=>set("referenceNo",e.target.value)} placeholder="Resolution / Reference No." className={cls}/><input type="date" value={f.meetingDate} onChange={e=>set("meetingDate",e.target.value)} title="Meeting Date" className={cls}/><textarea value={f.description} onChange={e=>set("description",e.target.value)} placeholder="Description / Details" className={cls+" sm:col-span-2 lg:col-span-2 min-h-[90px]"}/><input value={f.supportingDocument} onChange={e=>set("supportingDocument",e.target.value)} placeholder="Supporting Document / File Reference" className={cls}/><textarea value={f.remarks} onChange={e=>set("remarks",e.target.value)} placeholder="Remarks" className={cls}/><button className="sm:col-span-2 lg:col-span-4 bg-[#002344] text-white py-3 rounded-xl font-bold">Save History Event</button></form></div>{notice&&<div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl p-3 font-semibold">{notice}</div>}<div className="bg-white border rounded-2xl overflow-auto"><table className="w-full text-sm min-w-[1500px]"><thead className="bg-zinc-50"><tr>{["Date","Event Type","Event Title","Person / Committee","Previous Position","New Position","Resolution / Reference","Meeting Date","Description","Supporting Document","Remarks","Action"].map(h=><th key={h} className="p-3 text-left">{h}</th>)}</tr></thead><tbody className="divide-y">{existing.sort((a,b)=>String(a.recordDate).localeCompare(String(b.recordDate))).map(r=>{const d=r.data||{};return <tr key={r.id}>{[r.recordDate,d.eventType||r.recordType,d.title,d.personCommittee,d.previousRole,d.newRole,d.referenceNo,d.meetingDate,d.description,d.supportingDocument,d.remarks].map((v,i)=><td key={i} className="p-3">{v||"—"}</td>)}<td className="p-3"><button type="button" onClick={()=>archive(r.id)} className="px-3 py-1.5 rounded-lg border border-red-200 text-red-700 font-bold">Archive</button></td></tr>})}{!existing.length&&<tr><td colSpan="12" className="p-8 text-center text-zinc-500">No institutional history yet.</td></tr>}</tbody></table></div></SimpleOfficeCard>;
}

function OfficeHistory({rows,add,archive}){
 const existing=(rows||[]).filter(r=>r.module==="officeHistory"&&r.status!=="deleted"); const [f,setF]=useState({memberId:"",name:"",eventDate:new Date().toISOString().slice(0,10),changeType:"Appointment",previousRole:"",newRole:"",referenceNo:"",resolutionNo:"",meetingDate:"",description:"",remarks:""}); const [notice,setNotice]=useState(""); const set=(k,v)=>setF(x=>({...x,[k]:v}));
 const save=async e=>{e.preventDefault();if(!f.name.trim()||!f.newRole.trim()){setNotice("Name and New / Current Role required.");return;}const ok=await add("officeHistory",{recordDate:f.eventDate,recordType:f.changeType,status:"active",data:f});if(ok){setF({...f,name:"",memberId:"",previousRole:"",newRole:"",referenceNo:"",resolutionNo:"",meetingDate:"",description:"",remarks:""});setNotice("Office history event saved.");}};
 return <SimpleOfficeCard title="🏛️ प्रबंधकारिणी समिति / Office History" subtitle="किसने कब कौन सा पद संभाला — appointment, role change, resignation और removal का permanent history record."><div className="bg-white border rounded-2xl p-5"><form onSubmit={save} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3"><input value={f.memberId} onChange={e=>set("memberId",e.target.value)} placeholder="Member ID (if available)" className={cls}/><input value={f.name} onChange={e=>set("name",e.target.value)} placeholder="Full Name" required className={cls}/><input type="date" value={f.eventDate} onChange={e=>set("eventDate",e.target.value)} className={cls}/><select value={f.changeType} onChange={e=>set("changeType",e.target.value)} className={cls}>{["Appointment","Role Change / Transfer","Re-appointment","Additional Responsibility","Resignation","Removal","Relieving","Other"].map(x=><option key={x}>{x}</option>)}</select><input value={f.previousRole} onChange={e=>set("previousRole",e.target.value)} placeholder="Previous Role" className={cls}/><input value={f.newRole} onChange={e=>set("newRole",e.target.value)} placeholder="New / Current Role" required className={cls}/><input value={f.referenceNo} onChange={e=>set("referenceNo",e.target.value)} placeholder="Reference / File No." className={cls}/><input value={f.resolutionNo} onChange={e=>set("resolutionNo",e.target.value)} placeholder="Resolution No." className={cls}/><input type="date" value={f.meetingDate} onChange={e=>set("meetingDate",e.target.value)} className={cls}/><textarea value={f.description} onChange={e=>set("description",e.target.value)} placeholder="Details / Reason" className={cls+" sm:col-span-2 lg:col-span-2 min-h-[90px]"}/><textarea value={f.remarks} onChange={e=>set("remarks",e.target.value)} placeholder="Remarks" className={cls}/><button className="sm:col-span-2 lg:col-span-4 bg-[#002344] text-white py-3 rounded-xl font-bold">Save Office History</button></form></div>{notice&&<div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl p-3 font-semibold">{notice}</div>}<div className="bg-white border rounded-2xl overflow-auto"><table className="w-full text-sm min-w-[1300px]"><thead className="bg-zinc-50"><tr>{["Date","Member ID","Name","Change Type","Previous Role","New / Current Role","Reference","Resolution","Meeting Date","Details","Remarks","Action"].map(h=><th key={h} className="p-3 text-left">{h}</th>)}</tr></thead><tbody className="divide-y">{existing.sort((a,b)=>String(a.recordDate).localeCompare(String(b.recordDate))).map(r=>{const d=r.data||{};return <tr key={r.id}>{[r.recordDate,d.memberId,d.name,d.changeType||r.recordType,d.previousRole,d.newRole,d.referenceNo,d.resolutionNo,d.meetingDate,d.description,d.remarks].map((v,i)=><td key={i} className="p-3">{v||"—"}</td>)}<td className="p-3"><button type="button" onClick={()=>archive(r.id)} className="px-3 py-1.5 rounded-lg border border-red-200 text-red-700 font-bold">Archive</button></td></tr>})}{!existing.length&&<tr><td colSpan="12" className="p-8 text-center text-zinc-500">No office history yet.</td></tr>}</tbody></table></div></SimpleOfficeCard>;
}
