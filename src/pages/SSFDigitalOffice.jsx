...
function NotificationForm({types,onSave,members=[]}){
 const [f,setF]=useState({date:new Date().toISOString().slice(0,10),memberId:"",name:"",role:"",mobile:"",email:"",noticeStage:types[0]||"",subject:"",details:"",channel:"WhatsApp",expectedAction:"",expectedDate:"",responseStatus:"Not Applicable",responseDate:"",responseDetails:"",participationStatus:"Not Applicable",followUpDate:"",followUpResult:"",status:"active",relatedRecordId:"",evidenceRef:""});
 const set=(k,v)=>setF(x=>({...x,[k]:v}));
 const selectMember=(id)=>{const r=members.find(x=>String(x.id)===String(id));const d=r?.data||{};setF(x=>({...x,memberId:id,name:d.fullName||d.name||"",role:d.designation||d.organizationRole||d.role||"",mobile:d.mobile||d.mobileNo||"",email:d.email||""}));};
 ...
 const sendWhatsApp=()=>{if(!f.mobile){alert("Mobile number is required for WhatsApp.");return;}window.open("https://wa.me/"+String(f.mobile).replace(/\\D/g,"")+"?text="+encodeURIComponent(message()),"_blank","noopener,noreferrer");};
 const sendEmail=()=>{if(!f.email){alert("Email address is required for Email.");return;}window.location.href="mailto:"+f.email+"?subject="+encodeURIComponent(f.subject||"SSF Official Communication")+"&body="+encodeURIComponent(message());};
 const handleSubmit=e=>{e.preventDefault();onSave({recordDate:f.date,recordType:f.noticeStage,status:f.status,data:f,sendAction:"Saved communication record"});};
 return <form onSubmit={handleSubmit} className="p-5 bg-zinc-50 border-b grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
  <div className="sm:col-span-2 lg:col-span-4 rounded-2xl border bg-white p-4"><div className="font-black text-[#002344] mb-3">Recipient / प्राप्तकर्ता</div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
   <select value={f.memberId} onChange={e=>selectMember(e.target.value)} className={cls}><option value="">Select Existing Member</option>{members.map(r=>{const d=r.data||{};return <option key={r.id} value={r.id}>{d.fullName||d.name||r.recordId} — {d.designation||d.organizationRole||d.role||""}</option>})}</select>
   {input("name","Name / नाम",true)}{input("mobile","Mobile / WhatsApp")}{input("email","Email")}
   {input("role","Role / Position")}
   <div className="sm:col-span-2 lg:col-span-4 text-xs text-zinc-500">Existing member चुनें या किसी अन्य व्यक्ति के लिए Name + Mobile/Email manually भरें। कम से कम Mobile या Email में से एक देना जरूरी है।</div>
  </div></div>
  ...
  <div className="sm:col-span-2 lg:col-span-4 flex flex-wrap gap-2 items-center"><button type="button" onClick={sendWhatsApp} className="bg-green-600 text-white px-4 py-3 rounded-xl font-bold">WhatsApp</button><button type="button" onClick={sendEmail} className="bg-[#123B5D] text-white px-4 py-3 rounded-xl font-bold">Email</button><button type="button" onClick={()=>{if(!f.mobile&&!f.email){alert("Please enter Mobile or Email.");return;}if(f.mobile)sendWhatsApp();if(f.email)setTimeout(sendEmail,350);}} className="bg-[#002344] text-white px-4 py-3 rounded-xl font-bold">WhatsApp + Email</button><span className="text-xs text-zinc-500">पहले communication भेजें, फिर <b>Save Record</b> करें ताकि history बनी रहे।</span><button type="submit" className="border border-[#123B5D] text-[#123B5D] px-5 py-3 rounded-xl font-bold">Save Record</button><button type="button" onClick={()=>onSave(null)} className="border px-5 py-3 rounded-xl font-bold">Cancel</button></div>
 </form>;
}
