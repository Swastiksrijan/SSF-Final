...
function ManagingCommittee({rows,add,archive,token}){
 ...
 const [f,setF]=useState(blank),[notice,setNotice]=useState(""),[saving,setSaving]=useState(false),[syncing,setSyncing]=useState(false),[editingId,setEditingId]=useState(null),[section,setSection]=useState("profile");
 const set=(k,v)=>setF(x=>({...x,[k]:v}));
 ...
 const editRecord=r=>{const d=r.data||{};setEditingId(r.id);setF({...blank,...d,designation:designations.includes(d.designation)?d.designation:"Other / Custom",customDesignation:designations.includes(d.designation)?"":(d.designation||"")});setSection("profile");setNotice("");window.scrollTo({top:0,behavior:"smooth"});};
 const resetForm=()=>{setEditingId(null);setF({...blank});setSection("profile");};
 const save=async e=>{e.preventDefault();if(saving)return;if(!f.fullName.trim()||!f.designation){setNotice("Name and designation are required.");return;}const role=f.designation==="Other / Custom"?f.customDesignation.trim():f.designation;if(!role){setNotice("Custom designation enter karein.");return;}const supplied=f.memberId.trim();if(!editingId&&supplied&&committeeRows.some(r=>String((r.data||{}).memberId||"").trim().toLowerCase()===supplied.toLowerCase())){setNotice("This Member ID already exists. Duplicate ID save nahi kiya gaya.");return;}const ids=committeeRows.map(r=>{const m=String((r.data||{}).memberId||"").match(/SSF-MBR-(\d+)/i);return m?Number(m[1]):0;});const memberId=supplied||("SSF-MBR-"+String(Math.max(0,...ids)+1).padStart(5,"0"));setSaving(true);const data={...f,designation:role,memberId,action:f.action||"Committee Member Register / Update"};delete data.customDesignation;const ok=editingId?await updateRecord(editingId,"managingCommittee",data):await add("managingCommittee",{recordDate:f.effectiveFrom||new Date().toISOString().slice(0,10),recordType:"Committee Member",status:f.status.toLowerCase(),data});if(ok){setF(blank);setEditingId(null);setNotice(editingId?"Managing Committee record updated successfully.":"Committee member record saved successfully.");}setSaving(false);};
