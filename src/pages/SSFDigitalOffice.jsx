...
const GOVERNANCE_MODULES = [
 ["govParticipation","Managing Committee Participation / प्रबंधकारिणी सहभागिता",FaUserTie],
 ["govMeetingNotices","Meeting Notice & Reminder Log / बैठक सूचना एवं अनुस्मारक",FaCalendarAlt],
 ["govFollowups","Governance Follow-up & Explanation / शासन संबंधी अनुवर्ती कार्य एवं स्पष्टीकरण",FaTasks],
 ["govActions","Administrative Actions & Decisions / प्रशासनिक कार्रवाई एवं निर्णय",FaBalanceScale],
 ["govCompliance","Statutory Registration & Compliance / वैधानिक पंजीयन एवं अनुपालन",FaFileAlt],
 ["govDocuments","Governing Documents & Amendments / शासी दस्तावेज़ एवं संशोधन",FaFileAlt],
 ["govCalendar","Compliance Calendar & Renewals / अनुपालन कैलेंडर एवं नवीनीकरण",FaCalendarAlt],
 ["govBody","Governing Body & Tenure Record / शासी निकाय एवं कार्यकाल अभिलेख",FaUserTie],
 ["govDecisions","Institutional Decisions & Resolutions Index / संस्थागत निर्णय एवं प्रस्ताव सूचकांक",FaBalanceScale],
 ["govCommunication","Official Communication Register / आधिकारिक संचार रजिस्टर",FaFileAlt],
 ["govRelations","External Organisations & Institutional Relations / बाहरी संस्थाएँ एवं संस्थागत संबंध",FaHandshake],
 ["govEvidence","Governance Records & Evidence Index / शासन अभिलेख एवं प्रमाण सूचकांक",FaHistory]
];
GOVERNANCE_MODULES.forEach(function(x){LABELS[x[0]]=x[1];});
 ...
 return <div className="min-h-screen bg-zinc-50 pt-28 pb-16 px-3 sm:px-6"><div className="max-w-[1500px] mx-auto">
  ...
  <div className="grid lg:grid-cols-[245px_1fr] gap-5">
   ...
   <main className="min-w-0">
    {active==="governanceHome"&&<GovernanceDashboard setActive={setActive}/>}
    ...
    {GOVERNANCE_MODULES.some(function(x){return x[0]===active;})&&<GovernanceRegister module={active} rows={rows} loading={loading} search={search} setSearch={setSearch} add={add} archive={archive}/>}
...
function GovernanceDashboard({setActive}){
 const cards=[
  ["govParticipation","Managing Committee Participation","प्रबंधकारिणी सहभागिता","9-member committee attendance, response and participation trail."],
  ["govMeetingNotices","Meeting Notice & Reminder Log","बैठक सूचना एवं अनुस्मारक","Notice, WhatsApp/email reminders, response and proof reference."],
  ["govFollowups","Governance Follow-up & Explanation","अनुवर्ती कार्य एवं स्पष्टीकरण","Follow-up, explanation sought, reply and pending matters."],
  ["govActions","Administrative Actions & Decisions","प्रशासनिक कार्रवाई एवं निर्णय","Documented action with decision and resolution reference."],
  ["govCompliance","Statutory Registration & Compliance","वैधानिक पंजीयन एवं अनुपालन","Registration, certificates, status, validity and due dates."],
  ["govDocuments","Governing Documents & Amendments","शासी दस्तावेज़ एवं संशोधन","Rules, bye-laws, objectives, amendments and versions."],
  ["govCalendar","Compliance Calendar & Renewals","अनुपालन कैलेंडर एवं नवीनीकरण","Renewals, expiry dates, responsibility and follow-up."],
  ["govBody","Governing Body & Tenure Record","शासी निकाय एवं कार्यकाल","Office-bearers, committee term and changes."],
  ["govDecisions","Institutional Decisions & Resolutions Index","संस्थागत निर्णय एवं प्रस्ताव","Decision references linked to meetings and resolutions."],
  ["govCommunication","Official Communication Register","आधिकारिक संचार","Important official correspondence and references."],
  ["govRelations","External Organisations & Institutional Relations","बाहरी संस्थाएँ एवं संबंध","Institutional relationships, partners and coordination."],
  ["govEvidence","Governance Records & Evidence Index","शासन अभिलेख एवं प्रमाण","Evidence/reference index for notices, attendance and decisions."]
 ];
 return <div className="space-y-5"><div className="bg-[#123B5D] text-white rounded-2xl p-6 sm:p-8"><div className="text-xs font-black uppercase tracking-[.2em] text-[#E8D39A]">Swastik Srijan Foundation</div><h2 className="text-3xl sm:text-4xl font-black mt-2">Governance, Compliance & Institutional Records</h2><p className="mt-2 text-white/75 max-w-3xl">शासन, अनुपालन एवं संस्थागत अभिलेख · A dedicated section based on the Foundation's governing rules. Existing 39 office modules remain separate and preserved.</p></div><div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">{cards.map(function(x){return <button key={x[0]} onClick={function(){setActive(x[0]);}} className="text-left bg-white border border-[#D8E3EA] rounded-2xl p-5 hover:shadow-md hover:-translate-y-0.5 transition"><div className="flex items-start justify-between gap-3"><div><div className="text-lg font-black text-[#123B5D]">{x[1]}</div><div className="text-sm font-semibold text-[#1F7A70] mt-1">{x[2]}</div></div><span className="text-xs font-black px-2 py-1 rounded-full bg-[#FFF8E7] text-[#123B5D]">OPEN</span></div><p className="text-sm text-zinc-500 mt-3 leading-6">{x[3]}</p></button>})}</div></div>;
}

function GovernanceRegister({module,rows,loading,search,setSearch,add,archive}){
 const [open,setOpen]=useState(false),[status,setStatus]=useState("all");
 const filtered=rows.filter(function(r){return status==="all"||String(r.status||"").toLowerCase()===status;});
 const fields={
  govParticipation:[["date","Meeting Date","date",true],["meetingType","Meeting Type","text",true],["memberName","Member Name","text",true],["designation","Designation","text",false],["meetingMode","Mode: Physical / Online","text",false],["noticeDate","Notice Date","date",false],["response","Response Status","text",false],["attendance","Attendance","text",true],["absenceReason","Reason / Explanation","text",false],["followup","Follow-up","text",false],["resolutionNo","Resolution / Minutes Ref.","text",false],["remarks","Remarks","textarea",false]],
  govMeetingNotices:[["date","Communication Date","date",true],["meetingDate","Meeting Date","date",true],["recipient","Member / Recipient","text",true],["mode","Mode","text",true],["subject","Subject","text",true],["communicationType","Notice / Reminder","text",true],["response","Response","text",false],["responseDate","Response Date","date",false],["proofReference","WhatsApp / Email / File Proof","text",false],["nextFollowup","Next Follow-up","date",false],["statusNote","Status / Remarks","textarea",false]],
  govFollowups:[["date","Follow-up Date","date",true],["person","Member / Person","text",true],["matter","Matter / Reference","text",true],["previousNotice","Previous Notice / Reminder Ref.","text",false],["followupMode","Follow-up Mode","text",false],["explanationRequired","Explanation Required?","text",false],["replyDate","Reply Date","date",false],["replySummary","Reply / Explanation Summary","textarea",false],["nextAction","Next Action","text",false],["statusNote","Status / Remarks","textarea",false]],
  govActions:[["date","Action Date","date",true],["person","Person / Committee Member","text",true],["matter","Matter","text",true],["actionType","Action Type","text",true],["previousReferences","Previous Notice / Follow-up Ref.","text",false],["explanation","Explanation / Reply","textarea",false],["decision","Decision","textarea",false],["resolutionNo","Resolution No.","text",false],["effectiveDate","Effective Date","date",false],["supportingRef","Supporting Document Ref.","text",false]],
  govCompliance:[["date","Record / Update Date","date",true],["category","Registration / Compliance","text",true],["authority","Authority / Department","text",false],["registrationNo","Registration / Reference No.","text",false],["issueDate","Issue Date","date",false],["validUntil","Valid Until","date",false],["dueDate","Renewal / Compliance Due Date","date",false],["statusNote","Status / Remarks","textarea",false]],
  govDocuments:[["date","Record Date","date",true],["documentType","Document Type","text",true],["title","Document Title","text",true],["version","Version","text",false],["effectiveDate","Effective From","date",false],["amendmentRef","Amendment / Resolution Ref.","text",false],["approval","Approved By / Authority","text",false],["fileReference","File / Document Reference","text",false],["statusNote","Status / Remarks","textarea",false]],
  govCalendar:[["date","Entry Date","date",true],["complianceItem","Compliance / Renewal Item","text",true],["dueDate","Due Date","date",true],["responsible","Responsible Person","text",false],["lastAction","Last Action / Follow-up","text",false],["nextAction","Next Action","text",false],["statusNote","Status / Remarks","textarea",false]],
  govBody:[["date","Record Date","date",true],["memberName","Name","text",true],["designation","Designation","text",true],["appointmentDate","Appointment / Election Date","date",false],["termFrom","Term From","date",false],["termTill","Term Till","date",false],["changeType","Change / Event","text",false],["resolutionNo","Resolution No.","text",false],["statusNote","Status / Remarks","textarea",false]],
  govDecisions:[["date","Decision Date","date",true],["subject","Decision / Subject","text",true],["meetingRef","Meeting Ref.","text",false],["resolutionNo","Resolution No.","text",false],["responsible","Responsible Person","text",false],["actionStatus","Action Status","text",false],["completionDate","Completion Date","date",false],["supportingRef","Document / File Ref.","text",false],["statusNote","Remarks","textarea",false]],
  govCommunication:[["date","Communication Date","date",true],["direction","Inward / Outward","text",true],["fromTo","From / To","text",true],["authority","Organisation / Authority","text",false],["subject","Subject","text",true],["referenceNo","Reference No.","text",false],["mode","Mode","text",false],["responseDue","Response / Due Date","date",false],["fileReference","File Reference","text",false],["statusNote","Status / Remarks","textarea",false]],
  govRelations:[["date","Record Date","date",true],["organisation","Organisation Name","text",true],["organisationType","Type","text",false],["contactPerson","Contact Person","text",false],["purpose","Purpose / Relationship","textarea",false],["agreementRef","MoU / Reference","text",false],["validUntil","Valid Until","date",false],["statusNote","Status / Remarks","textarea",false]],
  govEvidence:[["date","Evidence Date","date",true],["evidenceType","Evidence Type","text",true],["relatedModule","Related Register / Module","text",true],["recordReference","Record / Notice / Resolution Ref.","text",true],["person","Person / Subject","text",false],["fileReference","File / Link / Proof Reference","text",false],["description","Description","textarea",false],["statusNote","Remarks","textarea",false]]
 };
 const cfg=fields[module]||[]; const blank=Object.fromEntries(cfg.map(function(x){return [x[0],x[2]==="date"?new Date().toISOString().slice(0,10):""];})); const [f,setF]=useState(blank);
 useEffect(function(){setF(blank);},[module]);
 const save=async function(e){e.preventDefault();const data=Object.assign({},f);const recordDate=f.date||f.meetingDate||f.dueDate||new Date().toISOString().slice(0,10);const recordType=f.actionType||f.communicationType||f.documentType||f.complianceItem||f.evidenceType||f.meetingType||"Governance Record";const status=(f.statusNote||"").toLowerCase().includes("pending")?"pending":"active";const ok=await add(module,{recordDate,recordType,status,data});if(ok){setF(blank);setOpen(false);}};
 const title=(LABELS[module]||module).split(" / ")[0]; const hindi=(LABELS[module]||module).split(" / ").slice(1).join(" / ");
 return <div className="bg-white rounded-2xl border overflow-hidden"><div className="p-5 sm:p-7 border-b bg-gradient-to-r from-white to-[#FFF8E7]"><div className="text-xs font-black uppercase tracking-wider text-[#1F7A70]">Swastik Srijan Foundation · Governance & Compliance</div><h2 className="text-2xl sm:text-3xl font-black text-[#123B5D] mt-1">{title}</h2><div className="text-base font-semibold text-[#1F7A70] mt-1">{hindi}</div><p className="text-sm text-zinc-500 mt-2">{filtered.length} record(s) · structured institutional record · historical entries are preserved.</p><div className="flex flex-wrap gap-2 mt-4"><input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder="Search records" className="px-3 py-2.5 border rounded-xl w-56"/><select value={status} onChange={function(e){setStatus(e.target.value);}} className="px-3 py-2.5 border rounded-xl"><option value="all">All Status</option><option value="active">Active</option><option value="pending">Pending</option><option value="completed">Completed</option></select><button onClick={function(){setOpen(!open);}} className="bg-[#123B5D] text-white px-4 py-2.5 rounded-xl font-bold"><FaPlus className="inline mr-2"/> Add Record</button></div></div>{open&&<form onSubmit={save} className="p-5 bg-zinc-50 border-b grid sm:grid-cols-2 lg:grid-cols-4 gap-3">{cfg.map(function(x){return x[2]==="textarea"?<textarea key={x[0]} value={f[x[0]]||""} onChange={function(e){setF(Object.assign({},f,{[x[0]]:e.target.value}));}} placeholder={x[1]} required={x[3]} className={cls+" min-h-[90px] sm:col-span-2"}/>:<input key={x[0]} type={x[2]} value={f[x[0]]||""} onChange={function(e){setF(Object.assign({},f,{[x[0]]:e.target.value}));}} placeholder={x[1]} required={x[3]} className={cls}/>;})}<button type="submit" className="sm:col-span-2 lg:col-span-4 bg-[#123B5D] text-white py-3 rounded-xl font-bold">Save {title}</button></form>}<div className="overflow-auto"><table className="w-full text-sm min-w-[900px]"><thead className="bg-[#123B5D] text-white"><tr><th className="p-3 text-left">ID</th><th className="p-3 text-left">Date</th><th className="p-3 text-left">Primary Detail</th><th className="p-3 text-left">Reference / Status</th><th className="p-3 text-left">Action</th></tr></thead><tbody className="divide-y">{filtered.map(function(r){const d=r.data||{};const primary=d.memberName||d.person||d.recipient||d.organisation||d.title||d.subject||d.complianceItem||d.matter||d.evidenceType||r.recordType||"—";const ref=d.resolutionNo||d.referenceNo||d.recordReference||d.registrationNo||d.fileReference||"";return <tr key={r.id}><td className="p-3 font-bold text-[#123B5D]">{r.recordId}</td><td className="p-3 whitespace-nowrap">{r.recordDate?new Date(r.recordDate).toLocaleDateString("en-IN"):"—"}</td><td className="p-3"><b>{primary}</b><div className="text-xs text-zinc-500 mt-1">{d.mode||d.meetingMode||d.communicationType||d.designation||d.actionType||d.documentType||""}</div></td><td className="p-3"><div>{ref||"—"}</div><div className="text-xs font-semibold text-[#1F7A70] mt-1">{r.status||"active"}</div></td><td className="p-3"><button onClick={function(){archive(r.id);}} className="text-xs font-bold text-red-600">Archive</button></td></tr>})}{!filtered.length&&<tr><td colSpan="5" className="p-10 text-center text-zinc-400">No records yet.</td></tr>}</tbody></table></div></div>;
}
