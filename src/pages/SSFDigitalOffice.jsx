...
function Dashboard({summary}){
 ...
 <div className="grid lg:grid-cols-2 gap-5"><div className="bg-white rounded-2xl border border-[#D8E3EA] p-6 shadow-sm"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-[#123B5D] text-white flex items-center justify-center font-black">SSF</div><div><h2 className="text-xl font-black text-[#123B5D]">All required office modules</h2><p className="text-xs text-[#1F7A70] font-bold mt-1">पेपरलेस NGO कार्यालय</p></div></div><p className="text-zinc-500 mt-3">Members, Volunteers, Donors, Donations, Internship, Beneficiaries, Events/Camps, Projects, Documents, Expenses, all registers, reports, users and audit trail.</p><div className="mt-5 grid sm:grid-cols-2 gap-2 text-sm">{MODULES.filter(function(x){return x[0]!=="dashboard";}).map(function(x){return <div key={x[0]} className="bg-[#F5F8FA] border border-[#E3EBF0] rounded-lg px-3 py-2 font-semibold text-[#123B5D]">{x[1]}</div>;})}</div></div>
 <div className="bg-[#123B5D] rounded-2xl border border-[#123B5D] p-6 shadow-sm text-white"><div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-black">Automatic financial linking</h2><p className="text-sm text-[#D8EDE9] mt-1">एक ही database transaction में linked records</p></div><div className="px-3 py-1 rounded-full bg-[#FFF8E7] text-[#123B5D] text-xs font-black">LIVE</div></div><p className="text-[#D8EDE9] mt-4">Donation and expense workflows write linked contribution, cash/bank and ledger records in one database transaction.</p><div className="mt-5 space-y-2 text-sm font-bold text-white"><p>Donation → Donor → Contribution → Cash/Bank → Ledger → Receipt record</p><p>Expense → Expense Register → Cash/Bank → Ledger</p><p>Create/update/archive → Audit Trail</p></div><div className="mt-5 bg-[#FFF8E7] border border-[#E8D39A] rounded-xl p-4 text-sm text-[#5A4510]"><strong>Compliance:</strong> final statutory/tax treatment, 80G particulars and audit requirements must be verified with SSF's CA/tax advisor.</div></div></div></div>;
}
