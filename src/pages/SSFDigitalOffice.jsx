...
function ManagingCommittee({rows,add,updateRecord,archive,token}){
 ...
 const [f,setF]=useState(blank),[notice,setNotice]=useState(""),[saving,setSaving]=useState(false),[syncing,setSyncing]=useState(false),[editingId,setEditingId]=useState(null),[section,setSection]=useState("dashboard");
 const set=(k,v)=>setF(x=>({...x,[k]:v}));
 ...
 const roleCount={};active.forEach(r=>{const role=(r.data||{}).designation||"Member";roleCount[role]=(roleCount[role]||0)+1;});
 const findMember=id=>committeeRows.find(r=>String((r.data||{}).memberId)===id);
 const syncCurrent=async()=>{if(!token||syncing)return;setSyncing(true);setNotice("");try{
   const latest=[...committeeRows];
   for(const m of current){const existing=findMember(m.memberId);const data={...m,status:"Active",responsibilities:m.functionalResponsibility,action:"Current Committee Register"};if(existing){await updateRecord(existing.id,"managingCommittee",data);}else{await add("managingCommittee",{recordDate:m.effectiveFrom,recordType:"Committee Member",status:"active",data});}}
   const keep=new Set(current.map(x=>x.memberId));for(const r of latest){const id=String((r.data||{}).memberId||"");if(id&&!keep.has(id))await archive(r.id);}
   window.dispatchEvent(new CustomEvent("ssf-digital-office-refresh"));setNotice("Current Managing Committee data refreshed: 9 members. Older unmatched records were archived, not permanently deleted.");
 }catch(e){setNotice(e.message||"Committee data refresh failed.");}finally{setSyncing(false);}};
 const editRecord=r=>{const d=r.data||{};setEditingId(r.id);setF({...blank,...d});setSection("profile");window.scrollTo({top:0,behavior:"smooth"});};
 const reset=()=>{setEditingId(null);setF({...blank});setSection("profile");};
 const save=async e=>{e.preventDefault();if(saving)return;if(!f.fullName.trim()||!f.designation){setNotice("Name and designation are required.");return;}const role=f.designation==="Other / Custom"?(f.customDesignation||"").trim():f.designation;if(!role){setNotice("Custom designation required.");return;}setSaving(true);const data={...f,designation:role,status:"Active",action:"Committee Register Update"};delete data.customDesignation;const ok=editingId?await updateRecord(editingId,"managingCommittee",data):await add("managingCommittee",{recordDate:f.effectiveFrom||new Date().toISOString().slice(0,10),recordType:"Committee Member",status:"active",data});setSaving(false);if(ok){setNotice(editingId?"Committee member updated successfully.":"Committee member added successfully.");reset();}};
 const doAction=async action=>{if(!f.memberId||!f.fullName.trim()){setNotice("Select/edit a committee member first.");return;}const ok=await add("managingCommittee",{recordDate:f.effectiveFrom||new Date().toISOString().slice(0,10),recordType:action,status:["Resignation","Removal / Membership Cancellation","Replacement / Relieving"].includes(action)?"revoked":"active",data:{memberId:f.memberId,fullName:f.fullName,designation:f.designation,action,effectiveFrom:f.effectiveFrom,resolutionNo:f.resolutionNo,meetingDate:f.meetingDate,referenceNo:f.referenceNo,remarks:f.remarks}});if(ok)setNotice(action+" recorded in committee history.");};
 ...
 const roleCards=[["President","President / अध्यक्ष"],["Vice President","Vice President / उपाध्यक्ष"],["Secretary","Secretary / सचिव"],["Treasurer","Treasurer / कोषाध्यक्ष"],["Joint Secretary","Joint Secretary / संयुक्त सचिव"]];
 const renderProfile=()=> <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{[["memberId","Member ID"],["memberType","Member Type"],["fullName","Full Name"],["occupation","Occupation"],["gender","Gender"],["mobile","Mobile"],["email","Email"],["address","Address"],["city","City"],["state","State"],["pinCode","PIN Code"],["joiningDate","Joining Date"]].map(([k,l])=><div key={k}><label className="text-xs font-bold text-slate-500">{l}</label><input value={f[k]||""} onChange={e=>set(k,e.target.value)} className={cls+" mt-1"}/></div>)}</div>;
 const renderPosition=()=> <div className="grid sm:grid-cols-2 gap-4"><div><label className="text-xs font-bold text-slate-500">Organization Role</label><select value={f.designation} onChange={e=>set("designation",e.target.value)} className={cls+" mt-1"}>{designations.map(x=><option key={x}>{x}</option>)}</select></div><div><label className="text-xs font-bold text-slate-500">Functional Responsibility</label><input value={f.functionalResponsibility||""} onChange={e=>set("functionalResponsibility",e.target.value)} className={cls+" mt-1"}/></div><div className="sm:col-span-2"><label className="text-xs font-bold text-slate-500">Responsibilities / Duties</label><textarea value={f.responsibilities||""} onChange={e=>set("responsibilities",e.target.value)} className={cls+" mt-1 min-h-[120px]"}/></div></div>;
 const renderAppointment=()=> <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{[["appointmentDate","Appointment / Selection Date"],["referenceNo","Reference / File No."],["resolutionNo","Resolution No."],["meetingDate","Meeting Date"]].map(([k,l])=><div key={k}><label className="text-xs font-bold text-slate-500">{l}</label><input type={k==="meetingDate"||k==="appointmentDate"?"date":"text"} value={f[k]||""} onChange={e=>set(k,e.target.value)} className={cls+" mt-1"}/></div>)}<div className="lg:col-span-3"><label className="text-xs font-bold text-slate-500">Approval / Remarks</label><textarea value={f.remarks||""} onChange={e=>set("remarks",e.target.value)} className={cls+" mt-1 min-h-[100px]"}/></div></div>;
 const renderTenure=()=> <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">{[["effectiveFrom","Committee From"],["validTill","Committee Till"],["status","Committee Status"],["joiningDate","Membership Joining Date"]].map(([k,l])=><div key={k}><label className="text-xs font-bold text-slate-500">{l}</label>{k==="status"?<select value={f.status||"Active"} onChange={e=>set("status",e.target.value)} className={cls+" mt-1"}><option>Active</option><option>Ended</option><option>Role Changed</option><option>Resigned</option><option>Removed</option></select>:<input type="date" value={f[k]||""} onChange={e=>set(k,e.target.value)} className={cls+" mt-1"}/>}</div>)}</div>;
 useEffect(()=>{if(token)syncCurrent();},[token]);
 const selected=editingId?committeeRows.find(r=>r.id===editingId):null;
 return <div className="space-y-5">
  <Card className="overflow-hidden"><div className="bg-gradient-to-r from-[#002344] via-[#123B5D] to-[#1F7A70] text-white p-7"><div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5"><div><div className="text-xs uppercase tracking-[0.2em] text-white/60 font-bold">SSF Digital Office · Governance</div><h2 className="text-3xl font-black mt-2">Managing Committee <span className="text-white/70">/ प्रबंधकारिणी समिति</span></h2><p className="text-white/80 mt-2 max-w-3xl">Current committee dashboard, member responsibilities, appointments, tenure and complete governance history — all in one place.</p></div><button type="button" onClick={syncCurrent} disabled={syncing} className="bg-white text-[#002344] px-5 py-3 rounded-xl font-black shadow-sm">{syncing?"Refreshing…":"↻ Refresh Current Committee"}</button></div></div>
   <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 p-5 bg-slate-50">{[["Members",active.length],["Roles",Object.keys(roleCount).length],["Founder Members",active.filter(r=>String((r.data||{}).memberType)==="Founder Member").length],["Current Year","2026–27"],["History",committeeRows.length]].map(([a,b])=><div key={a} className="bg-white border rounded-xl p-4"><div className="text-xs text-slate-500 font-bold">{a}</div><div className="text-2xl font-black text-[#002344] mt-1">{b}</div></div>)}</div></Card>
  {notice&&<div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl p-3 font-semibold">{notice}</div>}
  <Card><div className="p-3 overflow-x-auto border-b"><div className="flex gap-2 min-w-max">{tabs.map(([id,label])=><button type="button" key={id} onClick={()=>setSection(id)} className={"px-4 py-2.5 rounded-xl text-sm font-bold "+(section===id?"bg-[#002344] text-white shadow-sm":"bg-slate-100 text-[#123B5D] hover:bg-slate-200")}>{label}</button>)}</div></div>
   <div className="p-5">
    {section==="dashboard"&&<div className="space-y-5"><div className="grid lg:grid-cols-5 gap-4">{roleCards.map(([role,label])=>{const r=active.find(x=>(x.data||{}).designation===role);const d=r?.data||{};return <button type="button" key={role} onClick={()=>r&&editRecord(r)} className="text-left bg-slate-50 hover:bg-white border rounded-2xl p-5 transition"><div className="text-xs font-black text-[#1F7A70] uppercase">{label}</div><div className="text-lg font-black text-[#002344] mt-2">{d.fullName||"Vacant"}</div><div className="text-sm text-slate-500 mt-1">{d.functionalResponsibility||"No responsibility recorded"}</div></button>})}</div><div className="grid lg:grid-cols-3 gap-4"><div className="lg:col-span-2 border rounded-2xl p-5"><div className="flex items-center justify-between gap-3"><div><h3 className="text-lg font-black text-[#002344]">Current Members</h3><p className="text-sm text-slate-500">Click a member to edit the official register.</p></div><button type="button" onClick={()=>setSection("register")} className="text-sm font-bold text-[#1F7A70]">View full register →</button></div><div className="mt-4 grid sm:grid-cols-2 gap-3">{active.map(r=>{const d=r.data||{};return <button type="button" key={r.id} onClick={()=>editRecord(r)} className="text-left border rounded-xl p-4 hover:shadow-sm transition"><div className="flex items-start justify-between gap-3"><div><div className="font-black text-[#002344]">{d.fullName||"—"}</div><div className="text-sm text-[#1F7A70] font-bold mt-1">{d.designation||"Member"}</div></div><span className="text-[11px] bg-emerald-50 text-emerald-700 px-2 py-1 rounded-full font-bold">ACTIVE</span></div><div className="text-sm text-slate-500 mt-2">{d.functionalResponsibility||"—"}</div></button>})}</div></div><div className="border rounded-2xl p-5"><h3 className="text-lg font-black text-[#002344]">Governance Snapshot</h3><div className="mt-4 space-y-3 text-sm"><div className="flex justify-between"><span>Committee size</span><b>{active.length}</b></div><div className="flex justify-between"><span>Founder members</span><b>{active.filter(r=>String((r.data||{}).memberType)==="Founder Member").length}</b></div><div className="flex justify-between"><span>Executive member</span><b>{active.filter(r=>(r.data||{}).designation==="Executive Committee Member").length}</b></div><div className="flex justify-between"><span>Other members</span><b>{active.filter(r=>["Member","Executive Committee Member"].includes((r.data||{}).designation)).length}</b></div><div className="pt-3 border-t text-xs text-slate-500">Member validity and committee tenure are maintained separately.</div></div></div></div></div>}
