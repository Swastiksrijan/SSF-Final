...
function NotificationForm({types,onSave}){
 const [f,setF]=useState({date:new Date().toISOString().slice(0,10),name:"",role:"",noticeStage:types[0]||"",subject:"",details:"",channel:"Official WhatsApp Group",expectedAction:"",expectedDate:"",responseStatus:"Not Applicable",responseDate:"",responseDetails:"",participationStatus:"Not Applicable",followUpDate:"",followUpResult:"",status:"active",relatedRecordId:"",evidenceRef:""});
 const set=(k,v)=>setF(x=>({...x,[k]:v})); const input=(k,p,req=false)=><input value={f[k]} onChange={e=>set(k,e.target.value)} placeholder={p} required={req} className={cls}/>; const area=(k,p)=><textarea value={f[k]} onChange={e=>set(k,e.target.value)} placeholder={p} className={cls+" min-h-[90px]"}/>;
 return <form onSubmit={e=>{e.preventDefault();onSave({recordDate:f.date,recordType:f.noticeStage,status:f.status,data:f});}} className="p-5 bg-zinc-50 border-b grid sm:grid-cols-2 lg:grid-cols-4 gap-3">{input("name","Person / Member",true)}{input("role","Role / Position")}{input("date","Date",true)}<select value={f.noticeStage} onChange={e=>set("noticeStage",e.target.value)} className={cls}>{types.map(x=><option key={x}>{x}</option>)}</select>{input("subject","Subject / Matter",true)}<select value={f.channel} onChange={e=>set("channel",e.target.value)} className={cls}><option>Official WhatsApp Group</option><option>WhatsApp</option><option>Email</option><option>Phone</option><option>Meeting</option><option>Letter</option><option>Other</option></select>{input("expectedAction","Expected Response / Action")}{input("expectedDate","Expected Date")}<select value={f.responseStatus} onChange={e=>set("responseStatus",e.target.value)} className={cls}><option>Not Applicable</option><option>Response Received</option><option>No Response</option><option>Partial Response</option></select><select value={f.participationStatus} onChange={e=>set("participationStatus",e.target.value)} className={cls}><option>Not Applicable</option><option>Present</option><option>Absent</option><option>Online Not Joined</option><option>Late</option><option>Task Not Completed</option><option>No Response</option></select>{input("responseDate","Response / Explanation Date")}{input("followUpDate","Follow-up Date")}{input("relatedRecordId","Related Meeting / Record ID")}{input("evidenceRef","Evidence / Document Reference")}<div className="sm:col-span-2 lg:col-span-4">{area("details","Notice / Message / Details")}</div><div className="sm:col-span-2 lg:col-span-4">{area("responseDetails","Response / Explanation / Follow-up Result")}</div><div className="sm:col-span-2 lg:col-span-4 flex gap-2"><button type="submit" className="bg-[#123B5D] text-white px-5 py-3 rounded-xl font-bold">Save Record</button><button type="button" onClick={()=>onSave(null)} className="border px-5 py-3 rounded-xl font-bold">Cancel</button></div></form>;
}
