...
function OnlineMeetings({token}){
 ...
 const saveMeeting=async(meetingLink,delivery={})=>{const data={date,title,meetingType:type,time,agenda,meetingLink,inviteeRecordIds:selected,platform:"Jitsi Meet",delivery};const r=await fetch(ENDPOINTS.DIGITAL_OFFICE_RECORDS,{method:"POST",headers,body:JSON.stringify({module:"onlineMeetings",recordDate:date,recordType:type,data})});const out=await r.json().catch(()=>({}));if(!r.ok)throw new Error(out.message||"Meeting save failed.");};
 const createMeeting=async()=>{if(!title.trim()||!date||!time){setNotice("Meeting title, date and time required.");return;}const emails=selectedEmails();if(!emails.length){setNotice("Kam se kam ek selected member ka email hona chahiye.");return;}setWorking(true);setNotice("Online meeting link banaya ja raha hai…");try{const r=await fetch(ENDPOINTS.DIGITAL_OFFICE_ONLINE_MEETING,{method:"POST",headers,body:JSON.stringify({title,date,time,agenda,emails})});const out=await r.json();if(!r.ok)throw new Error(out.message||"Online meeting creation failed.");setLink(out.meetingLink||"");await saveMeeting(out.meetingLink,{emailed:out.emailed,emailErrors:out.emailErrors||[]});setNotice(out.emailConfigured?("Meeting link created, saved and "+out.emailed+" member(s) ko email invitation bhej diya gaya."): "Meeting link created and saved. Email service configured nahi hai, isliye WhatsApp se share kar sakte hain.");}catch(e){setNotice(e.message||"Unable to create meeting.");}finally{setWorking(false);}};
 const shareWhatsApp=()=>{if(!link){setNotice("Pehle meeting create karein.");return;}const text="Swastik Srijan Foundation Samiti\\n\\nOnline Meeting: "+title+"\\nDate: "+date+"\\nTime: "+time+"\\nType: "+type+"\\nAgenda: "+(agenda||"As per meeting notice")+"\\n\\nJoin Meeting: "+link;window.open("https://wa.me/?text="+encodeURIComponent(text),"_blank");};
 return <div className="space-y-5">
  <div className="bg-white border rounded-2xl overflow-hidden">
   <div className="bg-[#002344] text-white p-6"><div className="flex items-center gap-3"><FaVideo className="text-2xl"/><div><h2 className="text-2xl font-black">Online Managing Committee Meeting</h2><p className="text-white/70 mt-1">Website se direct secure meeting link create karein aur selected members ko invitation bhejein.</p></div></div></div>
   <div className="p-6 grid md:grid-cols-2 gap-4">
    ...
    <div className="md:col-span-2 rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><div className="font-black text-[#002344]">1. Meeting Link</div><p className="text-xs text-zinc-600 mt-1">Google Cloud authorization ki zarurat nahi. Website ek unique Jitsi Meet room banakar link generate karegi.</p><div className="flex flex-wrap gap-2 mt-3"><button type="button" disabled={working} onClick={createMeeting} className="bg-[#002344] text-white px-5 py-3 rounded-xl font-bold disabled:opacity-50">{working?"Creating…":"Create Meeting & Send Link"}</button>{link&&<a href={link} target="_blank" rel="noreferrer" className="bg-emerald-600 text-white px-5 py-3 rounded-xl font-bold">Join Meeting</a>}</div>{link&&<div className="mt-3 bg-white border rounded-xl p-3 text-sm break-all"><b>Meeting Link:</b> {link}</div>}</div>
