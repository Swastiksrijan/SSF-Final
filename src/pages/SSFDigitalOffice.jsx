...
function MembersRegister({rows,add,archive}){
 ...
 return <SimpleOfficeCard title="👥 Members Register" subtitle="व्यक्ति की Master Details — membership identity ko role history se alag rakha gaya hai."><div className="bg-white border rounded-2xl p-5"><form onSubmit={save} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
  ...
  {archived.length>0&&<div className="bg-white border rounded-2xl p-4"><div className="font-bold text-[#123B5D] mb-3">Archived Meeting Records / आर्काइव बैठक रिकॉर्ड</div>{archived.map(r=><div key={r.id} className="flex flex-wrap items-center justify-between gap-3 border-b last:border-b-0 py-3"><div><b>{r.data?.meetingTitle||r.recordId}</b><div className="text-sm text-zinc-500">{r.recordDate||"—"} · {r.recordId}</div></div><button type="button" onClick={async()=>{await restore(r.id);setArchived(x=>x.filter(a=>a.id!==r.id));}} className="px-4 py-2 rounded-xl bg-[#123B5D] text-white font-bold">Restore / वापस लाएँ</button></div>)}</div>}
...
function MeetingResolutions({rows,add,archive,restore,token}){
 ...
 const loadArchived=async()=>{try{const r=await fetch(ENDPOINTS.DIGITAL_OFFICE_RECORDS+"?module=meetingResolutions&includeArchived=1",{headers:auth()});const d=await r.json();setArchived(Array.isArray(d)?d:[]);}catch(e){setArchived([]);}};
