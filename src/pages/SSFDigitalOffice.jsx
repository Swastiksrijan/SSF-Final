...
export default function SSFDigitalOffice(){
 ...
 const permanentDelete=async function(id){
  ...
  const r=await fetch(ENDPOINTS.DIGITAL_OFFICE_RECORDS+"/"+id+"/permanent",{method:"DELETE",headers:auth()});
  if(r.ok){setNotice("Record permanently deleted.");setArchived(x=>x.filter(a=>a.id!==id));load(active);}
 };
...
function MembersRegister({rows,add,archive}){
 ...
 return <SimpleOfficeCard title="👥 Members Register" subtitle="व्यक्ति की Master Details — membership identity ko role history se alag rakha gaya hai."><div className="bg-white border rounded-2xl p-5"><form onSubmit={save} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
  ...
  <div className="flex items-center justify-between gap-3 flex-wrap"><div className="font-bold text-[#123B5D]">Archived Records / आर्काइव रिकॉर्ड: {archived.length}</div><button type="button" onClick={loadArchived} className="px-4 py-2 rounded-xl border border-[#1F7A70] text-[#1F7A70] font-bold">View Archived / आर्काइव देखें</button></div>
  {archived.length>0&&<div className="bg-white border rounded-2xl p-4"><div className="font-bold text-[#123B5D] mb-3">Archived Meeting Records / आर्काइव बैठक रिकॉर्ड</div>{archived.map(r=><div key={r.id} className="flex flex-wrap items-center justify-between gap-3 border-b last:border-b-0 py-3"><div><b>{r.data?.meetingTitle||r.recordId}</b><div className="text-sm text-zinc-500">{r.recordDate||"—"} · {r.recordId}</div></div><div className="flex gap-2 flex-wrap"><button type="button" onClick={async()=>{await restore(r.id);setArchived(x=>x.filter(a=>a.id!==r.id));}} className="px-4 py-2 rounded-xl bg-[#123B5D] text-white font-bold">Restore / वापस लाएँ</button><button type="button" onClick={()=>permanentDelete(r.id)} className="px-4 py-2 rounded-xl border border-red-200 text-red-700 font-bold">Delete Permanently / स्थायी रूप से हटाएँ</button></div></div>)}</div>}
