...
function MeetingResolutions({rows,add,archive}){
 ...
 const [f,setF]=useState(blank),[notice,setNotice]=useState(""),[saving,setSaving]=useState(false),[syncing,setSyncing]=useState(false),[editingId,setEditingId],[query,setQuery]=useState(""),[sortBy,setSortBy]=useState("dateDesc")=useState(null);
 const set=(k,v)=>setF(x=>({...x,[k]:v}));
 const editRecord=r=>{const src=r.data||{};const seed=initialCommittee.find(m=>String(m.memberId).toUpperCase()===String(src.memberId||"").toUpperCase())||{};const d={...seed,...src};setEditingId(r.id);setF({...blank,...d,customDesignation:""});setNotice("");window.scrollTo({top:0,behavior:"smooth"});};
 ...
 const visibleRows=committeeRows.filter(r=>{const d=r.data||{};const q=query.trim().toLowerCase();if(!q)return true;return [d.memberId,d.fullName,d.name,d.mobile,d.phone].some(v=>String(v||"").toLowerCase().includes(q));}).sort((a,b)=>{const da=a.data||{},db=b.data||{};if(sortBy==="nameAsc")return String(da.fullName||da.name||"").localeCompare(String(db.fullName||db.name||""));if(sortBy==="idAsc")return String(da.memberId||"").localeCompare(String(db.memberId||""));if(sortBy==="roleAsc")return String(da.designation||"").localeCompare(String(db.designation||""));return String(db.effectiveFrom||db.appointmentDate||b.recordDate||"").localeCompare(String(da.effectiveFrom||da.appointmentDate||a.recordDate||""));});
 ...
 const save=async e=>{e.preventDefault();if(saving)return;if(!f.fullName.trim()||!f.designation){setNotice("Name and designation are required.");return;}const role=f.designation==="Other / Custom"?f.customDesignation.trim():f.designation;if(!role){setNotice("Custom designation enter karein.");return;}const data={...f,designation:role,action:"Committee Member Register / Update"};delete data.customDesignation;setSaving(true);let ok=false;if(editingId){ok=await updateRecord(editingId,"managingCommittee",data);if(ok){setF(blank);setEditingId(null);setNotice("Committee member record updated successfully.");}}else{const supplied=f.memberId.trim();if(supplied&&committeeRows.some(r=>String((r.data||{}).memberId||"").trim().toLowerCase()===supplied.toLowerCase())){setNotice("This Member ID already exists. Duplicate ID save nahi kiya gaya.");setSaving(false);return;}const ids=committeeRows.map(r=>{const m=String((r.data||{}).memberId||"").match(/SSF-MBR-(\\d+)/i);return m?Number(m[1]):0;});const memberId=supplied||"SSF-MBR-"+String(Math.max(0,...ids)+1).padStart(5,"0");data.memberId=memberId;ok=await add("managingCommittee",{recordDate:f.effectiveFrom||new Date().toISOString().slice(0,10),recordType:"Committee Member",status:f.status.toLowerCase(),data});if(ok){setF(blank);setNotice("Committee member record saved.");}}setSaving(false);};
 ...
 return <div className="space-y-5">
  ...
  <div className="bg-white border rounded-2xl overflow-hidden"><div className="p-5 border-b"><h3 className="text-xl font-black text-[#002344]">Committee Register & Complete History</h3><p className="text-sm text-zinc-500 mt-1">Appointment, role change and separation history can be maintained chronologically.</p><div className="mt-4 grid sm:grid-cols-[1fr_auto] gap-3"><div className="relative"><FaSearch className="absolute left-3 top-3 text-zinc-400"/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search by Member ID, Name or Mobile" className="w-full pl-9 pr-10 py-3 rounded-xl border border-zinc-200 bg-white outline-none focus:ring-2 focus:ring-[#002344]/20"/>{query&&<button type="button" onClick={()=>setQuery("")} className="absolute right-3 top-2.5 text-zinc-400 font-bold text-lg">×</button>}</div><select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="px-4 py-3 rounded-xl border border-zinc-200 bg-white font-semibold"><option value="dateDesc">Latest First</option><option value="dateAsc">Oldest First</option><option value="nameAsc">Name A–Z</option><option value="idAsc">Member ID</option><option value="roleAsc">Role A–Z</option></select></div></div><div className="p-5 space-y-3">{timeline.map(r=>{const d=r.data||{};return <div key={r.id} className="flex gap-3"><div className="w-24 shrink-0 text-xs font-black text-[#002344]">{d.effectiveFrom||r.recordDate||"Date unavailable"}</div><div className="relative flex-1 border-l-2 border-zinc-200 pl-5 pb-3"><div className="absolute -left-[7px] top-1 h-3 w-3 rounded-full bg-[#002344]"/><div className="bg-zinc-50 border rounded-xl p-4"><div className="font-black text-[#002344]">{d.fullName||"—"} · {d.designation||"—"}</div><div className="text-sm mt-1">{d.action||"Committee Member Register / Update"} · Status: {r.status}</div><div className="text-xs text-zinc-500 mt-2">Resolution: {d.resolutionNo||"—"} · Meeting: {d.meetingDate||"—"} · Reference: {d.referenceNo||"—"}</div></div></div></div>})}{!timeline.length&&<div className="p-8 text-center text-zinc-500">No committee records yet.</div>}</div>
   <div className="overflow-auto border-t"><table className="w-full text-sm min-w-[2500px]"><thead className="bg-zinc-50"><tr><th className="p-3 text-left">Member ID</th><th className="p-3 text-left">Member Type</th><th className="p-3 text-left">Organization Role</th><th className="p-3 text-left">Functional Responsibility</th><th className="p-3 text-left">Full Name</th><th className="p-3 text-left">Occupation / Profession</th><th className="p-3 text-left">Gender</th><th className="p-3 text-left">Father / Husband Name</th><th className="p-3 text-left">Mobile No.</th><th className="p-3 text-left">Email</th><th className="p-3 text-left">Address</th><th className="p-3 text-left">City</th><th className="p-3 text-left">State</th><th className="p-3 text-left">PIN Code</th><th className="p-3 text-left">Aadhaar (Optional)</th><th className="p-3 text-left">PAN (Optional)</th><th className="p-3 text-left">Joining / Admission Date</th><th className="p-3 text-left">Committee From</th><th className="p-3 text-left">Committee Till</th><th className="p-3 text-left">Committee Status</th><th className="p-3 text-left">Appointment / Selection Date</th><th className="p-3 text-left">Reference / File No.</th><th className="p-3 text-left">Resolution No.</th><th className="p-3 text-left">Meeting Date</th><th className="p-3 text-left">Responsibilities / Duties</th><th className="p-3 text-left">Action / Role History</th><th className="p-3 text-left">Remarks</th><th className="p-3 text-left">Action</th></tr></thead><tbody className="divide-y">{visibleRows.map(r=>{const src=r.data||{};const seed=initialCommittee.find(m=>String(m.memberId).toUpperCase()===String(src.memberId||"").toUpperCase())||{};const d={...seed,...src};Object.keys(seed).forEach(k=>{if(src[k]===""||src[k]===null||typeof src[k]==="undefined")d[k]=seed[k];});return <tr key={r.id}><td className="p-3 font-bold">{d.memberId||"—"}</td><td className="p-3">{d.memberType||"—"}</td><td className="p-3 font-bold">{d.designation||"—"}</td><td className="p-3">{d.functionalResponsibility||"—"}</td><td className="p-3 font-bold">{d.fullName||"—"}</td><td className="p-3">{d.occupation||"—"}</td><td className="p-3">{d.gender||"—"}</td><td className="p-3">{d.fatherHusbandName||"—"}</td><td className="p-3">{d.mobile||"—"}</td><td className="p-3">{d.email||"—"}</td><td className="p-3">{d.address||"—"}</td><td className="p-3">{d.city||"—"}</td><td className="p-3">{d.state||"—"}</td><td className="p-3">{d.pinCode||"—"}</td><td className="p-3">{d.aadhaar||"—"}</td><td className="p-3">{d.pan||"—"}</td><td className="p-3">{d.joiningDate||"—"}</td><td className="p-3">{d.effectiveFrom||"—"}</td><td className="p-3">{d.validTill||"—"}</td><td className="p-3">{d.status||"—"}</td><td className="p-3">{d.appointmentDate||"—"}</td><td className="p-3">{d.referenceNo||"—"}</td><td className="p-3">{d.resolutionNo||"—"}</td><td className="p-3">{d.meetingDate||"—"}</td><td className="p-3">{d.responsibilities||"—"}</td><td className="p-3">{d.action||"Committee Member Register / Update"}</td><td className="p-3">{d.remarks||"—"}</td><td className="p-3"><div className="flex gap-2"><button type="button" onClick={()=>editRecord(r)} className="px-3 py-2 rounded-lg bg-blue-600 text-white font-bold hover:bg-blue-700 mr-2">✏️ Edit</button><button type="button" onClick={()=>archive(r.id)} className="px-3 py-1.5 rounded-lg border border-red-200 text-red-700 font-bold hover:bg-red-50">Archive</button></div></td></tr>})}{!visibleRows.length&&<tr><td colSpan="28" className="p-8 text-center text-zinc-500">{query?"No matching committee records found.":"No committee records yet."}</td></tr>}</tbody></table></div>
  </div>
 </div>;
  const seedCommitteeRecords=async function(){
   try{
    const r=await fetch(ENDPOINTS.DIGITAL_OFFICE_RECORDS+"?module=managingCommittee",{headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"}});
    const existing=await r.json();
    const list=Array.isArray(existing)?existing:[];
    for(const member of initialCommittee){
     const match=list.find(x=>String((x.data||{}).memberId||"").toUpperCase()===String(member.memberId).toUpperCase() && (x.data||{}).action==="Committee Member Register / Update");
     const merged=Object.assign({},match&&match.data?match.data:{},member,{
      status:(match&&match.data&&match.data.status)||"Active",
      responsibilities:(match&&match.data&&match.data.responsibilities)||member.functionalResponsibility,
      action:"Committee Member Register / Update",
      remarks:(match&&match.data&&match.data.remarks)||"Initial Managing Committee register entry"
     });
     if(match){
      const missingOrChanged=["memberType","designation","functionalResponsibility","fullName","occupation","gender","fatherHusbandName","mobile","email","address","city","state","pinCode","aadhaar","pan","joiningDate","effectiveFrom","appointmentDate"].some(k=>String((match.data||{})[k]??"")!==String(member[k]??""));
      if(missingOrChanged){
       await fetch(ENDPOINTS.DIGITAL_OFFICE_RECORDS+"/"+match.id,{method:"PUT",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({module:"managingCommittee",data:merged,recordDate:match.recordDate||member.effectiveFrom||member.joiningDate,recordType:match.recordType||"Committee Member",status:match.status||"active"})});
      }
     }else{
      await fetch(ENDPOINTS.DIGITAL_OFFICE_RECORDS,{method:"POST",headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"},body:JSON.stringify({module:"managingCommittee",recordDate:member.effectiveFrom||member.joiningDate,recordType:"Committee Member",status:"active",data:merged})});
     }
    }
    const fresh=await fetch(ENDPOINTS.DIGITAL_OFFICE_RECORDS+"?module=managingCommittee",{headers:{Authorization:"Bearer "+token,"Content-Type":"application/json"}});
    const freshRows=await fresh.json();
    window.dispatchEvent(new CustomEvent("ssf-digital-office-refresh",{detail:{module:"managingCommittee",rows:Array.isArray(freshRows)?freshRows:[]}}));
   }catch(e){setNotice(e.message||"Unable to sync Managing Committee records.");}
  };ction ManagingCommittee({rows,add,archive,token}){
 const blank={memberId:"",memberType:"General Member",designation:"Member",customDesignation:"",fullName:"",fatherHusbandName:"",dob:"",occupation:"",gender:"",mobile:"",email:"",address:"",city:"",state:"",pinCode:"",aadhaar:"",pan:"",joiningDate:"",functionalResponsibility:"",status:"Active",effectiveFrom:"",validTill:"",appointmentDate:"",referenceNo:"",resolutionNo:"",meetingDate:"",responsibilities:"",remarks:""};
