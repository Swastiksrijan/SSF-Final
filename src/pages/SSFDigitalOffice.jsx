...
function OnlineMeetings({token}){
 const [title,setTitle]=useState("Managing Committee Meeting"),[date,setDate]=useState(new Date().toISOString().slice(0,10)),[time,setTime]=useState("19:00"),[type,setType]=useState("Managing Committee"),[agenda,setAgenda]=useState(""),[link,setLink]=useState(""),[members,setMembers]=useState([]),[selected,setSelected]=useState([]),[notice,setNotice]=useState(""),[googleConnected,setGoogleConnected]=useState(false),[working,setWorking]=useState(false);
 const headers={Authorization:"Bearer "+token,"Content-Type":"application/json"};
 useEffect(()=>{fetch(ENDPOINTS.DIGITAL_OFFICE_RECORDS+"?module=members",{headers}).then(r=>r.ok?r.json():[]).then(d=>setMembers(Array.isArray(d)?d:[])).catch(()=>setMembers([]));fetch(ENDPOINTS.DIGITAL_OFFICE_GOOGLE_STATUS,{headers}).then(r=>r.ok?r.json():{connected:false}).then(d=>setGoogleConnected(Boolean(d.connected))).catch(()=>setGoogleConnected(false));const params=new URLSearchParams(window.location.search);if(params.get("google")==="connected")setNotice("Google account connected. Ab website se direct Google Meet banaya ja sakta hai.");if(params.get("google")==="error")setNotice("Google connection failed: "+(params.get("message")||"Please try again."));},[]);
 const toggle=id=>setSelected(s=>s.includes(id)?s.filter(x=>x!==id):[...s,id]);
 const connectGoogle=async()=>{try{const r=await fetch(ENDPOINTS.DIGITAL_OFFICE_GOOGLE_CONNECT_URL,{headers});const d=await r.json();if(!r.ok)throw new Error(d.message||"Google connection unavailable.");window.location.href=d.url;}catch(e){setNotice(e.message);}};
 ...
 const createMeet=async()=>{if(!googleConnected){await connectGoogle();return;}if(!title.trim()||!date||!time){setNotice("Meeting title, date and time required.");return;}const emails=selectedEmails();if(!emails.length){setNotice("Kam se kam ek selected member ka email hona chahiye.");return;}setWorking(true);setNotice("Google Meet banaya ja raha hai…");try{const r=await fetch(ENDPOINTS.DIGITAL_OFFICE_GOOGLE_CREATE_MEETING,{method:"POST",headers,body:JSON.stringify({title,date,time,agenda,emails})});const out=await r.json();if(!r.ok)throw new Error(out.message||"Google Meet creation failed.");setLink(out.meetingLink||"");await saveMeeting(out.meetingLink,{addedMembers:out.addedMembers,emailed:out.emailed,emailErrors:out.emailErrors||[]});setNotice("Meeting created and saved. "+out.emailed+" member(s) ko email invitation bhej diya gaya.");}catch(e){setNotice(e.message||"Unable to create meeting.");}finally{setWorking(false);}};
 const shareWhatsApp=()=>{if(!link){setNotice("Pehle website se meeting create karein.");return;}const names=members.filter(r=>selected.includes(r.id)).map(r=>(r.data||{}).fullName||(r.data||{}).name||"Member");const text="Swastik Srijan Foundation Samiti\\n\\nOnline Meeting: "+title+"\\nDate: "+date+"\\nTime: "+time+"\\nType: "+type+"\\nAgenda: "+(agenda||"As per meeting notice")+"\\n\\nJoin Meeting: "+link;window.open("https://wa.me/?text="+encodeURIComponent(text),"_blank");};
 return <div className="space-y-5">
  <div className="bg-white border rounded-2xl overflow-hidden">
   <div className="bg-[#002344] text-white p-6"><div className="flex items-center gap-3"><FaVideo className="text-2xl"/><div><h2 className="text-2xl font-black">Online Managing Committee Meeting</h2><p className="text-white/70 mt-1">Website se direct Google Meet create karein aur selected members ko invitation bhejein.</p></div></div></div>
   <div className="p-6 grid md:grid-cols-2 gap-4">
    ...
    <div className="md:col-span-2 rounded-2xl border border-blue-100 bg-blue-50 p-4"><div className="font-black text-[#002344]">1. Google Meet Connection</div><p className="text-xs text-zinc-600 mt-1">{googleConnected?"Google account connected. Ab meeting website se direct create hogi.":"Pehli baar Connect Google karna hoga. Google authorization ke baad website direct meeting create karegi."}</p><div className="flex flex-wrap gap-2 mt-3"><button type="button" disabled={working} onClick={googleConnected?createMeet:connectGoogle} className="bg-[#002344] text-white px-5 py-3 rounded-xl font-bold disabled:opacity-50">{working?"Creating…":googleConnected?"Create Meeting & Send Link":"Connect Google"}</button>{link&&<a href={link} target="_blank" rel="noreferrer" className="bg-emerald-600 text-white px-5 py-3 rounded-xl font-bold">Join Meeting</a>}</div>{link&&<div className="mt-3 bg-white border rounded-xl p-3 text-sm break-all"><b>Meeting Link:</b> {link}</div>}</div>
    <div className="md:col-span-2"><div className="font-black text-[#002344] mb-2">2. Select Managing Committee Members</div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-64 overflow-auto">{members.length===0?<div className="text-sm text-zinc-500">No members found.</div>:members.map(r=>{const d=r.data||{};const email=d.email||"";return <label key={r.id} className={"flex items-center gap-2 border rounded-xl p-3 cursor-pointer "+(selected.includes(r.id)?"bg-zinc-100 border-[#002344]":"bg-white")}><input type="checkbox" checked={selected.includes(r.id)} onChange={()=>toggle(r.id)} disabled={!email}/><span><b>{d.fullName||d.name||"Member"}</b><small className="block text-zinc-500">{email||"No email — cannot send invitation"}</small></span></label>;})}</div></div>
    <div className="md:col-span-2 flex flex-wrap gap-2">{link&&<button type="button" onClick={shareWhatsApp} className="border px-5 py-3 rounded-xl font-bold">WhatsApp Share</button>}</div>
