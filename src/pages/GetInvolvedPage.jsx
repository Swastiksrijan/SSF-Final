...
export default function GetInvolvedPage() {
    ...
    useEffect(() => {
        ...
        if (hash === "movement" || hash === "partner") openForm(hash);
    ...
    return (
        <div className="w-full bg-white font-sans text-zinc-900 overflow-hidden">
            ...
            <section className="py-24 bg-zinc-50 border-y border-zinc-100 px-6"><div className="max-w-4xl mx-auto text-center"><motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="bg-white p-10 md:p-16 rounded-[4rem] shadow-xl border border-zinc-200"><div className="inline-block px-6 py-2 bg-zinc-100 rounded-full text-zinc-400 font-bold uppercase tracking-widest text-xs mb-8">🔒 CLARITY NOTE</div><h2 className="text-3xl font-bold text-[#002344] mb-8">Every role is unique</h2><div className="grid md:grid-cols-3 gap-6"><div className="p-6 bg-orange-50 rounded-2xl border border-orange-100"><p className="font-bold text-[#FF6600]">Volunteer</p><p className="text-zinc-400 text-sm mt-1">Gives Time & Skills</p></div><div className="p-6 bg-blue-50 rounded-2xl border border-blue-100"><p className="font-bold text-[#002344]">Member</p><p className="text-zinc-400 text-sm mt-1">Gives Direction</p></div><div className="p-6 bg-red-50 rounded-2xl border border-red-100"><p className="font-bold text-red-500">Supporter</p><p className="text-zinc-400 text-sm mt-1">Gives Resources</p></div></div><p className="mt-10 text-xl font-hindi text-zinc-500">हर role अलग है • कोई overlap नहीं • User खुद decide करता है।</p></motion.div></div></section>
            <section className="py-24 bg-white px-6"><div className="max-w-7xl mx-auto"><div className="text-center mb-16"><h2 className="text-4xl md:text-5xl font-bold text-[#002344]">Life at SSF | <span className="text-[#FF6600]">एक झलक</span></h2><p className="text-zinc-500 mt-4 text-lg">See the impact you'll be part of. Our community is built on joy, learning, and unity.</p></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"><div className="lg:col-span-2 rounded-[3rem] overflow-hidden shadow-xl h-[400px]"><img src="/images/children-unity-park.jpg" alt="Children Unity" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" /></div><div className="rounded-[3rem] overflow-hidden shadow-xl h-[400px]"><img src="/images/slum-outreach-children.jpg" alt="Slum Outreach" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" /></div><div className="rounded-[3rem] overflow-hidden shadow-xl h-[400px]"><img src="/images/classroom-mat-session.jpg" alt="Classroom Session" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" /></div><div className="rounded-[3rem] overflow-hidden shadow-xl h-[400px]"><img src="/images/rural-children-raising-hands.jpg" alt="Rural Children" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" /></div><div className="rounded-[3rem] overflow-hidden shadow-xl h-[400px]"><img src="/images/slum-area-outreach.jpg" alt="Outreach Efforts" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" /></div></div></div></section>
            ...
            {activeForm && <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm p-4 md:p-8 flex items-center justify-center" role="dialog" aria-modal="true"><div className="relative w-full max-w-3xl max-h-[92vh] overflow-y-auto bg-white rounded-[2rem] md:rounded-[3rem] shadow-2xl"><button type="button" onClick={closeForm} aria-label="Close form" className="sticky top-4 float-right mr-4 z-20 w-11 h-11 rounded-full bg-white/95 border border-zinc-200 text-zinc-600 flex items-center justify-center shadow-lg hover:text-red-600 hover:scale-105 transition-all"><FaTimes /></button><div className="clear-both">{activeForm === "volunteer" && <VolunteerForm />}{activeForm === "member" && <MemberForm />}{(activeForm === "movement" || activeForm === "partner") && <GetInvolvedInterestForm type={activeForm} onClose={closeForm} />}</div></div></div>}
        </div>
    );
}
