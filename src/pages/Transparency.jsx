...
export default function Transparency() {
    ...
    return (
        <div className="w-full font-inria bg-white overflow-hidden text-[#002344]">
            {/* Premium coded hero: framed content keeps every heading and line safely inside its visual board. */}
            <section className="relative min-h-[470px] lg:min-h-[540px] overflow-hidden bg-[#001326] text-white flex items-center py-10 lg:py-14">
                ...
                <div className="absolute inset-4 lg:inset-7 rounded-[32px] border border-white/10 pointer-events-none" />
                <div className="absolute inset-7 lg:inset-10 rounded-[26px] border border-[#fb8500]/20 pointer-events-none" />
                ...
                <div className="absolute right-[7%] top-[12%] hidden lg:block w-80 h-80 rounded-full border border-white/10 rotate-12" />
                <div className="absolute right-[9%] top-[17%] hidden lg:block w-64 h-64 rounded-[44px] border border-[#fb8500]/30 rotate-12 bg-white/[.03] backdrop-blur-sm" />
                <div className="container mx-auto px-8 lg:px-16 relative z-10 max-w-7xl">
                    <div className="grid lg:grid-cols-[minmax(0,1fr)_300px] gap-10 lg:gap-14 items-center">
                        <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .55 }} className="relative rounded-[30px] border border-white/15 bg-[#002344]/75 p-7 sm:p-9 lg:p-11 shadow-2xl backdrop-blur-md max-w-4xl">
                            <div className="absolute -top-px left-10 right-10 h-px bg-gradient-to-r from-transparent via-[#fb8500] to-transparent" />
                            <div className="absolute top-7 bottom-7 left-3 sm:left-4 w-1 rounded-full bg-gradient-to-b from-[#fb8500] via-orange-300 to-transparent" />
                            <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .45, delay: .08 }} className="ml-3 sm:ml-4 inline-flex items-center gap-2 rounded-full border border-[#fb8500]/30 bg-[#fb8500]/10 px-4 py-2 text-[10px] sm:text-xs font-black tracking-[.16em] uppercase text-orange-200">
                                <FaShieldAlt /> Documentary Transparency
                            </motion.div>
                            <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .55, delay: .12 }} className="ml-3 sm:ml-4 mt-6 text-4xl sm:text-5xl lg:text-[4.2rem] font-serif font-bold leading-[1.05] tracking-tight">
                                Transparency <span className="text-[#fb8500]">&amp;</span> Compliance
                            </motion.h1>
                            <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5, delay: .2 }} className="ml-3 sm:ml-4 mt-5 text-base sm:text-lg lg:text-xl text-slate-200 leading-relaxed max-w-2xl">
                                Building trust through openness, accountability, documented compliance, and responsible governance.
                            </motion.p>
                            <p className="ml-3 sm:ml-4 mt-4 text-sm sm:text-base text-orange-200 font-serif font-bold italic leading-relaxed">पारदर्शिता, जवाबदेही, दस्तावेज़ी अनुपालन और जिम्मेदार शासन के माध्यम से विश्वास का निर्माण।</p>
                            <div className="ml-3 sm:ml-4 mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div className="rounded-2xl border border-white/10 bg-white/[.07] px-4 py-3 backdrop-blur-md"><span className="block text-2xl font-black">2013</span><span className="text-xs text-slate-300">Registered Since</span></div>
                                <div className="rounded-2xl border border-white/10 bg-white/[.07] px-4 py-3 backdrop-blur-md"><span className="block text-2xl font-black">13</span><span className="text-xs text-slate-300">Annual Reports</span></div>
                                <div className="rounded-2xl border border-white/10 bg-white/[.07] px-4 py-3 backdrop-blur-md"><span className="block text-2xl font-black">7</span><span className="text-xs text-slate-300">Key Records</span></div>
                            </div>
                        </motion.div>
                        <motion.div initial={{ opacity: 0, scale: .9, rotate: 3 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} transition={{ duration: .7, delay: .18 }} className="hidden lg:flex relative h-[290px] w-[290px] mx-auto rounded-[42px] border border-white/15 bg-white/[.045] backdrop-blur-md items-center justify-center shadow-2xl">
                            <div className="absolute inset-5 rounded-[32px] border border-[#fb8500]/30" />
                            <div className="absolute inset-10 rounded-full border border-white/10" />
                            <div className="w-32 h-32 rounded-full bg-[#002344] border-2 border-[#fb8500]/70 flex items-center justify-center shadow-[0_0_55px_rgba(251,133,0,.18)]"><FaShieldAlt className="text-6xl text-[#fb8500]" /></div>
                            <div className="absolute bottom-7 left-0 right-0 text-center"><span className="text-[10px] font-black uppercase tracking-[.25em] text-orange-200">Trust • Records • Accountability</span></div>
                        </motion.div>
                    </div>
                </div>
            </section>
            ...
            <section className="py-16 lg:py-20 px-6 max-w-6xl mx-auto">
                    ...
                    {sections.map((section, idx) => (
                        <motion.div key={section.title} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .45, delay: idx * .04 }} className={`rounded-3xl border border-zinc-200 bg-white p-7 lg:p-8 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all ${idx === 1 ? 'lg:row-span-2' : ''}`}>
                            <div className="flex items-start gap-5">
                                ...
                                <div className="min-w-0"><h2 className="text-2xl lg:text-3xl font-serif font-bold text-[#002344]">{section.title}</h2><p className="text-[#fb8500] font-serif font-bold italic mt-1">{section.hindi}</p></div>
                            </div>
                ...
                <div className="rounded-3xl bg-gradient-to-br from-[#002344] to-[#063b63] text-white p-8 lg:p-10 relative overflow-hidden mb-16">
                    ...
                    <div className="relative grid md:grid-cols-3 gap-8 items-center"><div className="md:col-span-2"><p className="text-xs font-black uppercase tracking-[.2em] text-orange-200">Our transparency standard</p><h2 className="text-2xl lg:text-3xl font-serif font-bold mt-3">Evidence first. Clear records. Responsible governance.</h2><p className="text-slate-300 mt-3 leading-relaxed">Statutory records, annual reports and public documents are presented separately so visitors can verify information without searching through the website.</p></div><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white/10 border border-white/10 p-4"><FaCheckCircle className="text-[#fb8500]" /><p className="mt-2 font-bold">Documented</p></div><div className="rounded-2xl bg-white/10 border border-white/10 p-4"><FaChartLine className="text-[#fb8500]" /><p className="mt-2 font-bold">Year-wise</p></div></div></div>
                </div>
            </section>

            <section className="py-20 bg-[#f8fafc] border-y border-zinc-200"><div className="container mx-auto px-6 max-w-6xl"><div className="text-center max-w-3xl mx-auto mb-12"><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-zinc-200 text-[#fb8500] text-xs font-black uppercase tracking-widest shadow-sm"><FaShieldAlt /> Documentary Transparency</div><h2 className="text-3xl lg:text-4xl font-serif font-bold mt-5 text-[#002344]">Certificates &amp; Compliance</h2><p className="text-zinc-500 mt-4 leading-relaxed">Key statutory and organizational records are shared for public reference. Document links open the records maintained by the Foundation.</p></div><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">{documents.map((doc, idx) => <motion.div key={doc.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .35, delay: idx * .04 }} whileHover={{ y: -5 }} className="bg-white rounded-2xl p-6 border border-zinc-200 shadow-sm hover:shadow-xl hover:border-[#fb8500]/40 transition-all flex flex-col"><div className="flex items-start justify-between gap-4"><div className="w-12 h-12 rounded-xl bg-[#002344]/5 text-[#002344] flex items-center justify-center text-xl"><FaFileContract /></div><span className="text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full bg-orange-50 text-[#c65f00] border border-orange-100">{doc.status}</span></div><h3 className="text-xl font-bold mt-5 text-[#002344]">{doc.title}</h3><p className="text-sm text-[#fb8500] font-semibold mt-1">{doc.hindi}</p><p className="text-sm text-zinc-500 leading-relaxed mt-4 flex-1">{doc.description}</p><a href={doc.href} target="_blank" rel="noopener noreferrer" className="mt-6 inline-flex items-center justify-center gap-2 rounded-xl bg-[#002344] text-white px-4 py-3 text-sm font-bold hover:bg-[#fb8500] transition-colors">View Document <FaExternalLinkAlt className="text-xs" /></a></motion.div>)}</div><p className="text-center text-xs text-zinc-400 mt-8">For document verification or clarification, please contact the Foundation using the official contact details below.</p></div></section>

            <section className="py-20 bg-white"><div className="container mx-auto px-6 max-w-6xl"><div className="text-center max-w-3xl mx-auto mb-12"><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-50 border border-orange-100 text-[#c65f00] text-xs font-black uppercase tracking-widest"><FaFileAlt /> Year-wise record</div><h2 className="text-3xl lg:text-4xl font-serif font-bold mt-5 text-[#002344]">Annual Reports / वार्षिक प्रतिवेदन</h2><p className="text-zinc-500 mt-4 leading-relaxed">Year-wise annual reports of the Foundation are provided below for public reference and transparency.</p></div><div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">{annualReports.map((report, idx) => <motion.div key={report.year} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .3, delay: idx * .03 }} whileHover={{ y: -5 }} className="bg-white rounded-2xl p-5 border border-zinc-200 shadow-sm hover:shadow-xl hover:border-[#fb8500]/50 transition-all flex flex-col"><div className="w-12 h-12 rounded-xl bg-[#002344]/5 text-[#002344] flex items-center justify-center text-xl"><FaFileAlt /></div><h3 className="text-2xl font-bold mt-5 text-[#002344]">{report.year}</h3><p className="text-sm text-[#fb8500] font-semibold mt-1">वार्षिक प्रतिवेदन</p><a href={report.href} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex items-center justify-center gap-2 rounded-xl bg-[#002344] text-white px-4 py-3 text-sm font-bold hover:bg-[#fb8500] transition-colors">View Report <FaExternalLinkAlt className="text-xs" /></a></motion.div>)}</div></div></section>

            <section className="py-20 bg-[#f8fafc] border-y border-zinc-200"><div className="container mx-auto px-6 max-w-6xl"><div className="text-center mb-12"><h2 className="text-3xl lg:text-4xl font-serif font-bold text-[#002344]">Reports &amp; Disclosures</h2><p className="text-[#fb8500] font-serif font-bold italic mt-2">रिपोर्ट एवं सार्वजनिक प्रकटीकरण</p><p className="text-zinc-500 max-w-2xl mx-auto mt-4">Program highlights, financial summaries and impact documentation are maintained as part of the Foundation’s transparency practices.</p></div><div className="grid md:grid-cols-3 gap-6">{reports.map((report, idx) => <motion.div key={idx} whileHover={{ y: -5 }} className="bg-white p-8 rounded-2xl shadow-sm border border-zinc-200 hover:border-[#fb8500] transition-all group"><div className="w-14 h-14 mx-auto bg-blue-50 text-[#002344] rounded-full flex items-center justify-center text-2xl mb-6 group-hover:bg-[#fb8500] group-hover:text-white transition-colors"><FaFileAlt /></div><h3 className="text-xl font-bold text-[#002344]">{report.title}</h3><p className="text-sm text-[#fb8500] font-semibold mt-1">{report.hindi}</p><p className="text-sm text-zinc-400 mt-3">{report.note}</p></motion.div>)}</div></div></section>

            <section className="py-20 bg-white"><div className="container mx-auto px-6 max-w-5xl"><div className="text-center mb-10"><div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-50 border border-zinc-200 text-[#fb8500] text-xs font-black uppercase tracking-widest"><FaBuilding /> Public documents</div><h2 className="text-3xl font-serif font-bold text-[#002344] mt-5">Policies &amp; Public Documents</h2><p className="text-[#fb8500] font-serif font-bold italic mt-2">नीतियाँ और सार्वजनिक दस्तावेज़</p></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">{policies.map((policy) => <a key={policy.path} href={policy.path} className="bg-white rounded-xl border border-zinc-200 px-5 py-5 text-sm font-bold text-[#002344] hover:border-[#fb8500] hover:text-[#fb8500] transition-all shadow-sm">{policy.title} <span className="float-right">→</span></a>)}</div></div></section>
