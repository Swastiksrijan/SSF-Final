...
export default function AdminDashboard() {
  ...
  useEffect(() => { const sync = () => setLoggedIn(Boolean(localStorage.getItem(TOKEN_KEY))); window.addEventListener('storage', sync); const timer = window.setInterval(sync, 1000); return () => { window.removeEventListener('storage', sync); window.clearInterval(timer); }; }, []);
  ...
  return <main className="min-h-screen bg-[#f6f8fb]">
    ...
    <section id="admin-people" className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 pt-6 scroll-mt-24"><div className="rounded-[2rem] bg-white border border-zinc-100 p-6 shadow-sm"><p className="text-xs font-black uppercase tracking-widest text-[#ff6600]">People & Roles</p><h2 className="text-2xl font-black text-[#002344] mt-1">Members, Volunteers & Accounts</h2><p className="text-sm text-zinc-500 mt-1 mb-5">Approval and official ID/certificate issuance remain in the existing secure management area. Account administration is available below.</p><AdminPortalV2 /></div><AdminUserManagement /></section>
