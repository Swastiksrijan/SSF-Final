...
export const generateCertificate = async (name, role, date, certId = null, memberId = null, certificateType = "Participation") => {
  ...
  const normalizedRole = String(role || "volunteer").toLowerCase(); const normalizedCertificateType = String(certificateType || "Participation").trim().toLowerCase(); const isMember = normalizedRole === "member" || normalizedRole.includes("membership");
  ...
  const certificateHeading = isMember ? "CERTIFICATE OF MEMBERSHIP" : normalizedCertificateType.includes("participation") ? "CERTIFICATE OF PARTICIPATION" : normalizedCertificateType.includes("experience") ? "CERTIFICATE OF EXPERIENCE" : normalizedCertificateType.includes("completion") ? "CERTIFICATE OF COMPLETION" : normalizedCertificateType.includes("service") ? "CERTIFICATE OF SERVICE" : "CERTIFICATE OF APPRECIATION"; doc.setFont("times", "bold"); doc.setTextColor(navy); doc.setFontSize(isMember ? 27 : 30); doc.text(certificateHeading, centerX, 60, { align: "center" }); doc.setDrawColor(gold); doc.setLineWidth(0.8); doc.line(centerX - 50, 64, centerX + 50, 64);
