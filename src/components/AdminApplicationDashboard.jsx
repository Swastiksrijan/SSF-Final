...
export default function AdminApplicationDashboard() {
    ...
    const load = async () => {
        ...
        try {
            ...
            const [contacts, internships, interests] = await Promise.all([
                fetch(ENDPOINTS.ADMIN_CONTACTS, { headers }),
                fetch(ENDPOINTS.ADMIN_INTERNSHIPS, { headers }),
                fetch(ENDPOINTS.ADMIN_INTERESTS, { headers })
            ]);
            if ([contacts, internships, interests].some(r => r.status === 401)) throw new Error('Admin session expired. Please log in again.');
            if (![contacts, internships, interests].every(r => r.ok)) throw new Error('Some application data could not be loaded.');
            const [contactData, internshipData, interestData] = await Promise.all([contacts.json(), internships.json(), interests.json()]);
    ...
    const records = useMemo(() => {
        const contacts = data.contacts.map(x => ({ ...x, kind: 'contact', title: `${x.firstName || ''} ${x.lastName || ''}`.trim(), subtitle: x.email, detail: x.message, date: x.createdAt }));
        ...
        const interests = data.interests.map(x => ({ ...x, kind: 'interest', title: x.fullName, subtitle: x.email, detail: `${x.interestType === 'partner' ? 'CSR / Partnership' : 'Nation-Building Movement'} • ${x.phone} • ${x.message}`, date: x.createdAt }));
    ...
    const updateStatus = async (record, status) => {
        ...
        if (response.status === 401) return setError('Admin session expired. Please log in again.');
    ...
    return <section className="bg-white rounded-[2.5rem] shadow-sm border border-zinc-100 overflow-hidden">
        ...
        <div className="px-8 pt-5 flex flex-wrap gap-2">
            {tabs.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTab(id)} className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold ${tab === id ? 'bg-[#002344] text-white' : 'bg-zinc-100 text-zinc-600'}`}><Icon />{label}{id !== 'all' && <span className="opacity-70">({counts[id] || 0})</span>}</button>)}
        </div>
        {error && <div className="mx-8 mt-5 p-4 rounded-xl bg-red-50 text-red-700 font-semibold text-sm">{error}</div>}
        <div className="p-8 pt-5 overflow-x-auto">
            {loading ? <div className="py-12 text-center text-zinc-400">Loading applications…</div> : records.length === 0 ? <div className="py-12 text-center text-zinc-400">No submissions found.</div> : <table className="w-full text-left min-w-[900px]"><thead><tr className="bg-zinc-50 text-zinc-400 font-bold text-xs uppercase tracking-widest"><th className="px-5 py-4">Applicant</th><th className="px-5 py-4">Type / Details</th><th className="px-5 py-4">Submitted</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Message / Information</th></tr></thead><tbody className="divide-y divide-zinc-50 text-sm">{records.map(record => <tr key={`${record.kind}-${record.id}`} className="align-top"><td className="px-5 py-5"><p className="font-bold text-[#002344]">{record.title}</p><p className="text-xs text-zinc-500 mt-1">{record.subtitle}</p>{record.phone && <p className="text-xs text-zinc-500">{record.phone}</p>}</td><td className="px-5 py-5"><p className="font-bold capitalize">{record.kind === 'interest' ? (record.interestType === 'partner' ? 'CSR / Partnership' : 'Nation-Building Movement') : record.kind}</p><p className="text-xs text-zinc-500 mt-1 max-w-[260px]">{record.detail}</p>{record.resumePath && <a href={record.resumePath} target="_blank" rel="noreferrer" className="text-xs font-bold text-blue-700 inline-block mt-2">Resume</a>}</td><td className="px-5 py-5 whitespace-nowrap text-xs text-zinc-600">{dateText(record.date)}</td><td className="px-5 py-5"><select value={record.status || ''} onChange={e => updateStatus(record, e.target.value)} className="rounded-lg border border-zinc-200 px-2 py-2 text-xs font-bold"><option value="new">New</option><option value="contacted">Contacted</option><option value="closed">Closed</option>{record.kind === 'internship' && <><option value="pending">Pending</option><option value="reviewed">Reviewed</option><option value="selected">Selected</option><option value="rejected">Rejected</option></>}</select></td><td className="px-5 py-5 max-w-[360px] text-xs leading-5 text-zinc-600">{record.message || '—'}</td></tr>)}</tbody></table>}
        </div>
    </section>;
}
