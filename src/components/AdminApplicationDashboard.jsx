...
import { FaBriefcase, FaEnvelope, FaHandshake, FaHandsHelping, FaIdCard, FaUsers, FaTrash } from 'react-icons/fa';
...
const interestLabel = type => ({ volunteer: 'Volunteer', member: 'Membership', partner: 'CSR / Partnership', movement: 'Nation-Building Movement' }[type] || 'Interest');
const deleteEndpoints = (kind, id) => {
  ...
  return [`${API_BASE_URL}/api/admin/interests/${safeId}/delete`, `${API_BASE_URL}/api/admin/interests/${safeId}`];
};
...
export default function AdminApplicationDashboard() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [contacts, internships, interests] = await Promise.all(responses.map(r => r.json()));
      setData({
  ...
  const records = useMemo(() => {
    ...
    const interests = data.interests.map(x => ({
      ...x,
      kind: x.interestType === 'volunteer' ? 'volunteer' : x.interestType === 'member' ? 'member' : 'interest',
      title: x.fullName,
      subtitle: x.email,
      detail: `${interestLabel(x.interestType)}${x.category ? ` • ${x.category}` : ''} • ${x.phone}`,
      date: x.createdAt
    }));
    return [...contacts, ...internships, ...interests]
  ...
  const updateStatus = async (record, status) => {
    const token = localStorage.getItem(TOKEN_KEY);
    const endpoint = record.kind === 'contact'
      ? ENDPOINTS.ADMIN_CONTACT_STATUS(record.id)
      : record.kind === 'internship'
        ? ENDPOINTS.ADMIN_INTERNSHIP_STATUS(record.id)
        : ENDPOINTS.ADMIN_INTEREST_STATUS(record.id);
  ...
  const deleteRecord = async record => {
    ...
    try {
      const token = localStorage.getItem(TOKEN_KEY);
      const [postUrl, deleteUrl] = deleteEndpoints(record.kind, record.id);
      let response = await fetch(postUrl, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ permanent: true }) });
      if (response.status === 404) response = await fetch(deleteUrl, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
      const result = await response.json().catch(() => ({}));
      if (response.status === 401) { localStorage.removeItem(TOKEN_KEY); setAuthenticated(false); return; }
  ...
  const counts = {
    ...
    volunteer: data.interests.filter(x => x.interestType === 'volunteer').length,
    member: data.interests.filter(x => x.interestType === 'member').length,
  ...
  return <section className="bg-white rounded-[2.5rem] shadow-sm border border-zinc-100 overflow-hidden">
    <div className="p-8 border-b border-zinc-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
      <div><p className="text-xs font-black uppercase tracking-widest text-[#FF6600]">Application Records</p><h2 className="text-2xl font-black text-[#002344] mt-1">Applications & Participation</h2><p className="text-sm text-zinc-500 mt-1">Review submissions. Approving a Volunteer or Member issues the official ID and certificate record.</p></div>
    ...
    <div className="p-8 pt-5 overflow-x-auto">{loading ? <div className="py-12 text-center text-zinc-400">Loading applications…</div> : records.length === 0 ? <div className="py-12 text-center text-zinc-400">No submissions found.</div> : <table className="w-full text-left min-w-[1100px]"><thead><tr className="bg-zinc-50 text-zinc-400 font-bold text-xs uppercase tracking-widest"><th className="px-5 py-4">Applicant</th><th className="px-5 py-4">Type / Category</th><th className="px-5 py-4">Submitted</th><th className="px-5 py-4">Status / Action</th><th className="px-5 py-4">Message / Information</th></tr></thead><tbody className="divide-y divide-zinc-50 text-sm">{records.map(record => <tr key={`${record.kind}-${record.id}`} className="align-top"><td className="px-5 py-5"><p className="font-bold text-[#002344]">{record.title}</p><p className="text-xs text-zinc-500 mt-1">{record.subtitle}</p>{record.phone && <p className="text-xs text-zinc-500">{record.phone}</p>}</td><td className="px-5 py-5"><p className="font-bold">{record.kind === 'interest' || record.kind === 'volunteer' || record.kind === 'member' ? interestLabel(record.interestType) : record.kind}</p><p className="text-xs text-zinc-500 mt-1 max-w-[300px]">{record.detail}</p>{record.resumePath && <a href={`${API_BASE_URL}${record.resumePath}`} target="_blank" rel="noreferrer" className="text-xs font-bold text-blue-700 inline-block mt-2">Open Resume</a>}</td><td className="px-5 py-5 whitespace-nowrap text-xs text-zinc-600">{dateText(record.date)}</td><td className="px-5 py-5"><div className="flex flex-col gap-2"><select disabled={busy === `${record.kind}-${record.id}`} value={record.status || ''} onChange={e => updateStatus(record, e.target.value)} className="rounded-lg border border-zinc-200 px-2 py-2 text-xs font-bold"><option value="new">New</option>{record.kind === 'internship' ? <><option value="pending">Pending</option><option value="reviewed">Reviewed</option><option value="selected">Selected — Issue ID & Letter</option><option value="completed">Completed — Issue Certificate</option><option value="rejected">Rejected</option></> : record.kind === 'volunteer' || record.kind === 'member' ? <><option value="contacted">Contacted</option><option value="approved">Approved — Issue ID</option><option value="rejected">Rejected</option><option value="closed">Closed</option></> : <><option value="contacted">Contacted</option><option value="closed">Closed</option></>}</select><button onClick={() => deleteRecord(record)} disabled={!!busy} className="inline-flex items-center justify-center gap-2 bg-red-600 text-white hover:bg-red-700 px-3 py-2 rounded-xl font-bold text-xs"><FaTrash />{busy === `${record.kind}-${record.id}` ? 'Working…' : 'Delete Permanently'}</button></div></td><td className="px-5 py-5 max-w-[360px] text-xs leading-5 text-zinc-600">{record.message || '—'}</td></tr>)}</tbody></table>}</div>
  </section>;
}
