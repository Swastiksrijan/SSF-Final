...
const Header = () => {
  ...
  return <header className="fixed top-0 left-0 right-0 z-50 font-sans">
    ...
    {menuOpen && <div className="md:hidden fixed inset-0 top-[116px] bg-white z-40 overflow-y-auto pb-20"><nav className="p-6">
      ...
      {navItems.filter(item => item.name !== "Our Work").map(item => <Link key={item.name} to={item.path} onClick={closeMenu} className="block py-4 border-b text-lg font-bold text-[#002344]">{item.name}</Link>)}
