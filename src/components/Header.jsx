    ...
    <header className="fixed top-0 left-0 right-0 z-50 font-sans">
          ...
          <div className="flex items-center justify-end gap-3 sm:gap-4 text-[11px] sm:text-xs font-semibold text-white/90 whitespace-nowrap overflow-x-auto no-scrollbar">
            ...
            {user ? (
              <div className="relative flex items-center">
                <button type="button" onClick={() => setAccountOpen(!accountOpen)} className="flex items-center gap-2 hover:text-[#FF6600] transition-colors" aria-expanded={accountOpen}><FaUserCircle /><span className="max-w-[150px] truncate">{user.fullName || user.email}</span><IoIosArrowDown className={`transition-transform ${accountOpen ? "rotate-180" : ""}`} /></button>
        ...
        <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-5 lg:gap-7">
            ...
            <div className="relative group">
              <button type="button" className="flex items-center gap-1.5 py-2 text-sm font-bold tracking-wide text-[#002344] hover:text-[#FF6600] transition-all">Get Involved <IoIosArrowDown className="text-xs transition-transform group-hover:rotate-180" /></button>
              <div className="absolute top-full left-1/2 -translate-x-1/2 w-64 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                <div className="bg-white border border-zinc-100 shadow-2xl rounded-2xl overflow-hidden p-2">
                  {getInvolvedItems.map((item) => <Link key={item.name} to={item.path} className="block px-4 py-3 text-[13px] font-semibold text-zinc-600 hover:text-[#FF6600] hover:bg-zinc-50 rounded-xl transition-all">{item.name}</Link>)}
                </div>
              </div>
            </div>
            <Link to="/GetInvolved"><button className="flex items-center gap-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white px-6 py-2.5 rounded-full font-black hover:from-green-700 hover:to-emerald-700 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 -translate-y-0.5">🚀 {lang === "en" ? "JOIN US" : "हमसे जुड़ें"}</button></Link>
          </nav>
      ...
      {menuOpen && <div className="md:hidden fixed inset-0 top-[116px] bg-white z-40 overflow-y-auto pb-20 animate-in slide-in-from-right duration-500"><nav className="flex flex-col p-6 space-y-2">
        ...
        <div className="border-b border-zinc-50">
          <button type="button" onClick={() => toggleMobileSubMenu("Get Involved")} className="w-full flex items-center justify-between py-4 text-lg font-bold text-[#002344]">Get Involved <IoIosArrowDown className={`transition-transform ${mobileSubMenu === "Get Involved" ? "rotate-180" : ""}`} /></button>
          {mobileSubMenu === "Get Involved" && <div className="mb-3 pl-4 space-y-1 border-l-2 border-[#FF6600]/10">{getInvolvedItems.map((item) => <Link key={item.name} to={item.path} onClick={closeMenu} className="block py-3 text-base font-medium text-zinc-600 hover:text-[#FF6600]">{item.name}</Link>)}</div>}
        </div>
