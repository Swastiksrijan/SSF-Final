...
const FORM_CONFIG = {
  volunteer: { title: "Volunteer", icon: FaUserFriends, eyebrow: "Serve with SSF", description: "Use the profile details already saved in your SSF account. No separate identity-document upload is required here." },
...
function VolunteerPortalForm({ user }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    setError("");
    if (!user?.id || !user?.fullName || !user?.email || !user?.phone) return setError("Please complete your My Profile details first."), setState("error");
    if (!user?.profilePhotoPath) return setError("Please add your profile photo in My Profile before applying as a volunteer."), setState("error");
    if (form.message.trim().length < 10) return setError("Please tell us briefly how you would like to contribute."), setState("error");
    setState("submitting");
    try {
      const response = await fetch(ENDPOINTS.PORTAL_VOLUNTEER, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ accountId: user.id, name: user.fullName, email: user.email, phone: user.phone, volunteer_type: form.volunteerType, message: form.message.trim() }) });
      ...
      setState("success");
    } catch (err) { setError(err.message || "Unable to submit the volunteer application. Please try again."); setState("error"); }
  };

  if (state === "success") return <div className="text-center py-12 px-5"><FaCheckCircle className="mx-auto text-5xl text-emerald-500" /><h3 className="text-2xl font-black text-[#002344] mt-5">Volunteer Application Submitted</h3><p className="text-zinc-500 mt-2">Your profile photo and account details have been used. No separate identity document was requested.</p><p className="text-sm text-zinc-400 mt-2">Reference: <strong>{reference}</strong></p></div>;
  ...
  return <form onSubmit={submit} className="p-5 md:p-7 space-y-5" noValidate>
    ...
    <button disabled={state === "submitting"} className="w-full rounded-2xl bg-[#002344] text-white py-4 font-black text-base hover:bg-[#ff6600] transition inline-flex items-center justify-center gap-3 disabled:opacity-60">{state === "submitting" ? <><FaSpinner className="animate-spin" /> Sending…</> : <>Submit Volunteer Application <FaArrowRight /></>}</button>
...
function ParticipationForm({ kind }) {
  ...
  const submit = async (e) => {
    ...
    setState("submitting");
    try { const type = kind === "partnership" || kind === "skills" ? "partner" : "movement"; const response = await fetch(ENDPOINTS.INTEREST, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ type, fullName: form.fullName.trim(), email: form.email.trim().toLowerCase(), phone: form.phone.trim(), category: form.category.trim(), message: form.message.trim() }) }); const result = await response.json().catch(() => ({})); if (!response.ok) throw new Error(result.message || "Unable to submit your request."); setState("success"); } catch (err) { setError(err.message || "Unable to submit your request. Please try again."); setState("error"); }
  };
...
export default function UserParticipationForms({ openForm, onClose, user }) {
  ...
  const renderForm = () => { if (openForm === "membership") return <MemberForm />; if (openForm === "volunteer") return <VolunteerPortalForm user={user} />; if (["activities", "partnership", "skills"].includes(openForm)) return <ParticipationForm kind={openForm} />; if (openForm === "internship") return <InternshipForm />; return <DonorForm />; };
